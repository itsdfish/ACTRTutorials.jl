...
The Simple Retrieval Model and Guessing Mixture Model are restrictive in the sense that they do not permit learning or forgetting. The probability of retrieval for each trial was the same regardless of what was experienced on previous trials. Although this restriction may be reasonable for situations in which activation has stabilized through long-term learning, this restriction will not be reasonable in situations in which new information must be learned. To overcome this weakness, we will develop a new model that includes ACT-R's base-level learning mechanisms to capture the dynamics of learning and forgetting.

## Task

The simulated task we will use is quite simple: On each trial, the model will attempt to retrieve a single fact to provide an answer. As before, if the activation of the memory is above the threshold, it will retrieve the fact and respond correctly. Otherwise, the model will experience a retrieval failure, resulting in an incorrect answer (no guessing possible). After responding, corrective feedback is provided to reinforce memory. The inter-stimulus interval is uniformly distributed between 5 and 15 seconds to provide a richer data set for estimating the decay parameter. 
...
$\begin{align}
\mathbf{r}_i = \{(\textrm{word},c_{s,\rm imaginal}(\rm cue)\} 
\end{align}$

where $i$ is a trial index, $c_{s,\rm imaginal}(\rm word)$ is the word value of the chunk in the imaginal buffer. The slot set for the retrieval request is defined as $Q_r = \rm \{cue\}$ 

### Response Mapping

Let $y_i$ $\in \{1,2,\cdots, 10\, \emptyset \}$ be the response given by the model. The response mapping is defined as:


"""

# ╔═╡ 72292a4a-9573-4713-ab34-cae5ffc9b77c
md"
$$y_i =
  \begin{cases}
    \emptyset      & \textrm{failure}\\
    c_r(\rm number)
  \end{cases}$$
...
$\begin{align}
R_i = \{\mathbf{c}_m \in M : c_m(\rm cue) = y_i\}
\end{align}$
...
# ╔═╡ 88f63794-7dad-493b-8aed-b4fb30a82db6
md"
...
$\begin{align}
\textrm{bll}_m =  \textrm{LN}\left(\sum_{j=1}^{N_m}t_{mj}^{-d}\right)
\end{align}$

where $t_{mj}$ is the time lag between the current time and the $j^{th}$ use of the chunk $m$, $N_m$ is the number of times the chunk has been used, and $d \in [0,1]$ is a decay parameter with a default value of .5. We will also set blc $=0$ for simplicity. 

One problem with base-level activation is that it becomes computationally intensive to compute as the chunk history grows. We will use the hybrid approximation introduced by Petrov (2006) and further validated by Fisher, Houpt and Gunzelmann (2018) to increase the speed of the model. The logic behind the hybrid approximation is that the most recent uses of a chunk drives the majority of the activation dynamics, whereas earlier uses of the chunk contribute much less and can be approximated with an equal temporal spacing assumption. The hybrid approximation is defined as:
...
Let's think through some properties of base-level activation. All other things being equal, (1) increasing $L$ or $t_k$ will decrease activation due to forgetting over the passage of time, (2) increasing $N$ will increase activation due to more learning, and (3) increasing $d$ will decrease activation due to faster forgetting. 

The plot below shows how activation changes after 10 uses of a chunk over a 60 second. Each line corresponds to a different value of activation. Activation reduces approximately to the log of memory uses when $d=0$ because activation does not decay. However, when $d > 0$, activation increases briefly following the retrieval of a chunk and gradually decreases until the next retrieval. Each time a chunk is retrieved, activation increases even more than the previous time.
"
...
1. Activation for a single memory increases with use and decays with time according to base-level activation
2. N is incremented by 1 on each trial to reflect corrective feedback
3. The following parameters are fixed: $\tau=.5$, $\sigma = .4$ and blc $= 0$
"

# ╔═╡ 3e687184-209d-4fec-8cf8-4b20af8605a0
md"
# Generate Data

We will use the following function to generate data for parameter estimation. The `simulate` function accepts the following inputs

- parms: a `NamedTuple` of fixed parameters
- n_trials: the number of simulated trials
- d: the decay parameter.

The annotated code below shows how an an `ACTR` object is created and the number of correct responses are generated for both target and foil trials. 
"

# ╔═╡ f104d208-609c-467e-94b5-cccff36aeaa0
md"
`simulate` outputs a `NamedTuple` with all the information needed to reconstruct base-level learning. The following is an example trial:

example_trial = (recent = [10.06], lags = [9.05], L = 19.12, N = 2, resp = true)

The keys in the example trial are defined as follows:

- recent: a vector that trackets the most recent use (in this case a single element becase k=1)
- lags: a vector that tracks the lags between recent and lifetime L
- L: the lifetime of the chunk defined as the time since the chunk was created
- N: the number of uses of the chunk
- resp: indicates whether a response was made (true) or not (false)
"

# ╔═╡ e9f8a787-e0a7-44e4-8884-00d7d1a9566e
md"

The simulated data are generated in the code block below
"

# ╔═╡ a1017c06-f130-4de5-a58a-8ec5d70ac60a
md"

The plot below shows the expected learning effect over the course of 50 trials, aggregated into 5 consecutive blocks. Although the data are somewhat noisy, there is a general trend of increasing accuracy from block 1 through block 5. 
"
...
The code below shows the log likelihood function for the simple learning model. The function `computeLL` accepts the following arguments:

- parms: a `NamedTuple` of fixed parameters
- data: a `NamedTuple` of trial-level response information
-  $d$: the decay parameter. 

The data are organized as an array of `NamedTuples`, with each `NamedTuple` containing the necessary information for each trial: the accuracy of the response, the number times the chunk was used, the chunk's lifetime at retrieval, and the lag since the chunks last use. The first four lines initialize the model objects and the log likelihood value. The following for loop iterates through the data for each trial. On each iteration, the chunk is updated/modified with the data and the log likelihood of a response is computed where r is the retrieval probablity and f is the retrieval failure probability.
"
...
$d \sim \textrm{beta}(5,5)$

$\theta_{r,i} = \Pr(Y_i = y_i \mid d ; \mathbf{r}_i)$
 
$R_{i} \sim \textrm{bernoulli}(\theta_{r,i})$

You may have noticed that the Binomial distribution is replaced with a Bernoulli distribution. The Bernoulli distribution is a special case of the Binomial distribution when $N = 1$. The sum of Bernoulli random variables is a Binomial random variable so long as $\theta$ is constant (e.g. the data are identically and independently distributed). However, since $\theta_r$ is not constant as a result of learning and forgetting, the response and $\theta_r$ is identified by the trial index,$i$. The prior distribution for decay follows a Beta distribution centered at .5.


In computer code, the model is specified as follows:
"

# ╔═╡ 9d14d94f-93dc-42ae-af6c-62c93cef6aeb
@model model(data, parms) = begin
    d ~ Beta(5, 5)
    data ~ Retrieval(d, parms)
end

# ╔═╡ 3c8aa20b-d3d6-4046-9963-1c2165976d09
md"
## Estimate Parameters
...
The trace plot and autocorrelation plots in the first and second panels, respectively, show good mixing and low autocorrelation. In the third panel, the density plot shows that the posterior distribution of $d$ is centered near the true value of .5. Taken together, these results suggest the we are able to estimate the decay parameter properly. 
"
...
manifest_format = "2.0"
project_hash = "9befefa37dd177dab68193bbbad7d44a217ccee8"
