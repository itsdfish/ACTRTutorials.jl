...
function plot_fan(preds, trial, resp; options...)
    ...
    title = get_title()
    p = @df fanEffect histogram(:MeanRT, group = (:fanPlace, :fanPerson),
        ylabel = "Density",
        xaxis = font(6), yaxis = font(6), grid = false, norm = true, color = :grey,
        leg = false, size = (350, 300),
        titlefont = font(6), title = title, layout = 9, xlims = (1, 3), ylims = (0, 7),
        bins = 15; options...)
    return p
end

plot_fan!(p1, preds, trial::String, resp; options...) =
    plot_fan!(p1, preds, Symbol(trial), Symbol(resp); options...)
...
function scatter_fan(preds, parm, trial, resp; options...)
    fanEffect = filter(x -> (x.trial == trial) && (x.resp == resp), preds)
    title = get_title()
    p = @df fanEffect scatter(cols(parm), :MeanRT, group = (:fanPlace, :fanPerson),
        xlabel = string(parm),
        ylabel = "Mean RT", xaxis = font(6), yaxis = font(6), grid = false,
        color = :grey, leg = false, size = (800, 600),
...
function scatter_accuracy(preds, parm, trial; options...)
    ...
    title = get_title()
    p = @df fanEffect scatter(cols(parm), :accuracy, group = (:fanPlace, :fanPerson),
        ylabel = "Accuracy",
        xlabel = string(parm), xaxis = font(6), yaxis = font(6), grid = false,
        color = :grey, leg = false,
        size = (800, 600), titlefont = font(6), layout = 9, ylims = (0.5, 1),
        title = title; options...)
    return p
end
...
function plot_accuracy(preds, trial; options...)
    fanEffect = filter(x -> x.trial == trial, preds)
    title = get_title()
    p = @df fanEffect histogram(:accuracy, group = (:fanPlace, :fanPerson),
        ylabel = "Density",
        xaxis = font(6), yaxis = font(6), grid = false, norm = true, color = :grey,
        leg = false, size = (350, 300),
        titlefont = font(6), layout = 9, xlims = (0.5, 1), ylims = (0, 8), bins = 10,
        title = title; options...)
    return p
end
