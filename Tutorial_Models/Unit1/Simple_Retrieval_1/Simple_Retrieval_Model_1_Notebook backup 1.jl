...
- Simple Retrieval Model 1: the model can produce a correct response by retrieving the correct chunk or commit an error of omission via a retrieval failure
- [Simple Retrieval Model 2](./open?path=$path_u1_2): the model can produce a correct response by retrieving the correct chunk, commit an error of comission via partial matching, or commit an error of omission via a retrieval failure
- [Simple Retrieval Model 3](./open?path=$path_u1_3): a mixture model that either retrieves a chunk or guesses following a retrieval error. Thus, only errors of comission are possible and retrieval failures are not directly observed. 

Mirroring Unit 1, [Unit 2](./open?path=$path_u2_1) will demonstrate how to create analogous models of joint choice-reaction time using the [Lognormal Race process](./open?path=$path_lognormal). 
...
As previously noted, we will use variations of a simple paired associates task throughout Unit 1 and Unit 2. In the paired associates task, subjects learn arbitrary letter number pairs, such as

- (X, 2)
- (M, 5)

In the test phase, a letter is presented and the subject must recall the associated number and press the corresponding key on the keyboard. In this particular version of the task, subjects can respond "I don't know" if they are unable to remember the number. 
"""

# ╔═╡ ba30fa84-5ee2-44ed-94aa-86cb208ffeaa
md"
# Simple Retrieval Model 1

The Simple Retrieval model is an extremely simple ACT-R model of memory retrieval based on the binomial coin flip example in the introductory tutorial. Recall that the binomial likelihood function is: 

```math
\begin{align}
\mathcal{L}(\theta; k, n)  = {n \choose k} \theta^k (1-\theta)^{n-k}
\end{align}
```

where $k$ is the number of successes in $n$ trials and $\theta$ is the probability of success. Our basic approach to defining a likelihood function for the Simple Retrieval Model will be to express the binomial likelihood in terms of ACT-R memory retrieval mechanisms. In the Simple Retrieval model, the parameter $\theta$ represents the probability of retrieving a memory, which is based on from ACT-R's retrieval mechanism.
"

# ╔═╡ 7bef18ab-f7c5-47f5-9df5-6c6dc1548713
Markdown.parse("

As noted below, some details of the model, such as encoding and knowledge representation, are omitted from the code implementation without affecting the predictions. The purpose of ommiting these details is to simply the model implimentation. However, we will specify these details here to provide a theoretical justification for the model. In what follows, we will introduce the theoretical components formally. See the tutorial on [notation](./open?path=$path_notation) for details and explanation.
"
)

# ╔═╡ 6b557969-67f1-499a-ae12-43d9f62cbcf6
md"""
...
```math
\begin{align}
\mathbf{c}_{i,\rm imaginal} = \{ (\rm letter, v_i)\}
\end{align}
```

### Retrieval Request

In computational versions of ACT-R, a retrieval request is specified in the right hand side of a production rule under the buffer command`+retrieval>`. The retrieval request for trial $i$ is defined as:
```math
\begin{align}
...
As the name implies, the model the model is rather simple. A retrieval request $\mathbf{r}$ is submitted to the declarative memory buffer.  The correct memory is retrieved if its activation is above a retrieval threshold, $\tau$. Otherwise a memory retrieval failure occurs, prompting the model to omit a response, or equivalently, indicate "I don't know" by pressing the designated key.

### Activation

According to ACT-R, the probability of retrieving a chunk from memory is an increasing function the chunk's activation. Activation is governed by several processes, including the dynamics of learning and forgetting, similarity, context, and random variability. Typically, a component is included in the model if it has an appreciable influence on the task at hand. Otherwise, unnecessary components can be omitted for the sake of simplicity. We will use a simplified version consisting of two components:

```math
\begin{align}
a_{m} = \text{blc} + \epsilon_m
\end{align}
```

where $m$ indexes each chunk, blc is the base-level constant, and $\epsilon_m \sim \rm normal(0, \sigma)$ is normally distributed noise. For our present purposes, we can interpret blc as a stable level of activation achieved through asymptotic learning. As a matter of convention, we will designate the variable $\mu_m$ to represent the deterministic component of activation. In the present case, $\mu_m = \rm blc$. In general, however, $\mu_m$ may consist of other deterministic components.

### Retrieval Probability

The probability of retrieval $\theta_r$ is can be approximated with a soft max function (sometimes called Luce's choice rule) as defined in (Weaver, 2008): 

```math
\begin{align}
 \theta_r = \Pr(\mathbf{c}_m \mid \tau; \mathbf{r}_i)  \approx \frac{e^{\frac{\mu_m}{\sigma}}}{e^{\frac{\tau}{\sigma}} + e^{\frac{\mu_m}{\sigma}}}
\end{align}
```
where $\tau$ is the threshold parameter, $\sigma = s\sqrt{2}$ controls the noise and $s$ is the logistic scale parameter often uses in simulation based models of ACT-R. As shown in the plot below, the parameters are not mutually identifiable due to the simple structure of the model. The values within each diagonal band correspond to regions of the space spanned by blc and $\tau$ for which the predicted retrieval probability is the same. For this reason, we will estimate only the threshold parameter $\tau$ and will fix the remaining parameters to $\rm blc = 1.5$ and $s = .4$. We will set the true value of $\tau$ to .5. 

"""

# ╔═╡ f0578176-71f7-4e5b-80af-e0431122a18e
md"
### Likelihood Function

The likelihood function for Simple Retrieval Model 1 is formed by re-expressing the Binomial likelihood function in terms of ACT-R parameters. We define $K$ as a random variable representing the number of successful retrievals, $n$ as the number of trials, and $\theta$ as the rretrieval probability. The likelihood is written as:

$\mathcal{L}(\theta; k, n) = {n \choose k} \theta^{n} (1-\theta)^{n-k}$
"

# ╔═╡ 907f087e-f600-4d20-a73b-37fdf7751526
Markdown.parse("
### The effect of retrieval threshold on retrieval probability

It is often informative to reason about the effect different values of a parameter will have on the predictions: as \$\\tau\$ decreases the probability of retrieval increases (e.g.  \$e^{\\frac{\\tau}{\\sigma}}\$ will approach zero). This should make intuitive sense: a memory will be easier to retrieve if the required activation for retrieval is lower. Retrieval probability is plotted against \$\\tau\$ in the code blocks below. Before proceeding, we will activate the project environment, load the required packages and seed the random number generator with a value to make the results reproducable. See the tutorial on the [Julia programming language](./open?path=$path_julia) for details about syntax.
")
...
1. Each relevant chunk has the same mean activation
2. Activation fluctuates only due to noise
3. The correct chunk is retrieved if its activation is above $\tau$
4. The probability of retrieval $\theta$ is  independent and identical for all chunks under consideration
"

# ╔═╡ e442bbca-6db8-4133-a547-ba48681bcced
md"
## Generate Data
...
1. It calls `compute_prob`, which returns the retrieval probability
2. `compute_prob` creates the model object and then computes a retrieval probability $\theta$ using the function `retrieval_prob` 
3. `simulate` generates a random number of correct trials from a binomial distribution using `θ` and n_trials as `inputs`

You will notice that we ignored many details of the model, such as knowledge representation and encoding processes, as these do not change the predictions. Instead, we will treat the model as a statistical abstraction in which only the number of successful retrievals are needed for the likelihood function."

# ╔═╡ 32f479f8-4774-40b4-83a3-e56a64c7c017
function compute_prob(parms, n_trials; τ)
...
In the code below, we define a custom distribution object for the Simple Retrieval Model 1. Creating a new distribution object involves creating a new struct that is a subtype of `ContinuousUnivariateDistribution` and extending the `logpdf` function from Distributions.jl. The struct contains fields for the retrieval threshold, $\tau$, the number of trials, $n$, and the fixed parameters, parms (fields are typed generically with `T1`, `T2`, `T3` to make it compatible with automatic differentiation.

"

# ╔═╡ f1e0d33e-8da0-40cf-9e26-ba8c2a3a7b20
Markdown.parse("
Turing interfaces with the custom likelihood distribution through the overloaded `logpdf` function. The inputs are passed to `computeLL`, where the heavy lifting occurs. `computeLL` is set up similarly to the simulation function above called `simulate`. The primary differences are (1) we initialize activation at zero based on the type of \$\\tau\$, and (2) the probability of a successful retrieval is passed to `logpdf` instead of `rand`.  Please refer to the background tutorial for the package [Turing](./open?path=$path_mcmc) for details on MCMC sampling and creating custom distribution types.
")

# ╔═╡ f3b682ec-b82a-4ce5-a038-9c42e98101a2
md"
## Define Likelihood Function

In the code below, we define a custom distribution object for the Simple Retrieval Model 1. Creating a new distribution object involves creating a new struct that is a subtype of `ContinuousUnivariateDistribution` and extending the `logpdf` function from Distributions.jl. The struct contains fields for the retrieval threshold, $\tau$, the number of trials, $n$, and the fixed parameters, parms (fields are typed generically with `T1`, `T2`, `T3` to make it compatible with automatic differentiation.

Turing interfaces with the custom likelihood distribution through the overloaded `logpdf` function. The inputs are passed to `computeLL`, where the heavy lifting occurs. `computeLL` is set up similarly to the simulation function above called `simulate`. The primary differences are (1) we initialize activation at zero based on the type of $\tau$, and (2) the probability of a successful retrieval is passed to `logpdf` instead of `rand`.
"

# ╔═╡ 35611253-65e7-4409-a688-7c9db47df177
md"
## Define Model

Our next step is to formulate the model and the prior distribution over $\tau$. In practice, we do not know the exact value of a parameter. However, we typically have some general idea so that we can quantify our uncertainly with a distribution over possible values. Since we are using simulated data, we will center the prior distributions over the true value as a matter of convention throughout this tutorial. The rationale is that it allows us to assess whether the model is behaving as expected without the potential confounding influence of the prior distribution. Under these conditions, we typically expect the true value to be within the 95 $\%$ credible interval and some reduction of the spread of the posterior distribution compared to the prior distribution. The Simple Retrieval model is defined as follows:

```math
\begin{align*}
\tau \sim \rm normal(.5,.5) \\
\theta_{r} = \Pr(\mathbf{c}_m \mid \tau; \mathbf{r}_i)  \\
k \sim \rm binomial(n_{t},\theta_r)
\end{align*}
```

The first line states that the parameter $\tau$ has a normal prior distribution with a mean $\mu =.5$ and a standard deviation $\sigma = .5$. The second line above states that the probability of retrieval $\theta_r$ is the probability that activation for chunk $m$ is above the retreival threshold, $\tau$. Finally, the third line states that the number of successful retrievals $n_r$, follows a binimal distribution with $n$ trials and probability of success, $\theta_r$. In Turing, the model is written as follows:
"

# ╔═╡ 83a072db-1500-445e-bcd0-7a55edf57bcb
@model model(k, n_trials, parms) = begin
    τ ~ Normal(0.5, 0.5)
    k ~ Retrieval(τ, n_trials, parms)
end

# ╔═╡ df34fb84-3bcc-4cc4-b516-fe4f793fb8df
md"
## Estimate Parameters
...
(If this is your first time running the code, it will take a few moments to compile)
"
...
manifest_format = "2.0"
project_hash = "02833431a6d73c7356091d09e7b5361f0f601ba8"
