...
n_chains = 4
chain = sample(model(rts, fixed_parms), specs, MCMCThreads(), n_samples, n_chains, progress=true)
#######################################################################################
#                                         Plot
#######################################################################################
pyplot()
...
preds = vcat(preds...)
histogram(preds, norm=true, grid=false, leg=false, size=(800,400), xlabel="Mean", ylabel="Density", 
...
preds = vcat(preds...)
histogram(preds, norm=true, grid=false, leg=false, size=(800,400), xlabel="Standard Deviation", ylabel="Density", 
...
preds = vcat(preds...)
histogram(preds, norm=true, grid=false, leg=false, size=(800,400), xlabel="Skewness", ylabel="Density", 
...
preds = vcat(preds...)
histogram(preds, norm=true, grid=false, leg=false, size=(800,400), xlabel="Proportion of Lapses", ylabel="Density", 
    xlims=(0,.1), color=:darkgrey, title="Posterior Predictive of Lapses", xaxis=font(font_size),
    yaxis=font(font_size))
