...
In the previous tutorial Simple RT Model 2, we developed a simple memory model of reaction time which always generated a correct response. In many cases, however, assuming that errors are extremely rare or do not occur is untenable. One way that we can incorporate errors into the model is through retrievail failures. A retrieval failure is an error of omission in which no memory is retrieved. Within the model, this occurs when there are no chunks whose activation values meet or exceed the retrieval threshold. We will develop such a model using a family of models called the Lognormal Race model, which formally specify how response options race towards a threshold that triggers a response. 

## Task

The task is similar to the one described in the tutorial [Simple RT Model 3](./open?path=$path_u2_3).  In this task, a cue is presented to a particpant who must use the cue to recall a specific fact and response with the answer. The task does not permit the participant to answer `I don't know`. In such cases, the participant must guess the correct answer.
")

# ╔═╡ 3d5bb409-55bf-4c1d-a5be-93c2c71f10c3
md"""
# Simple RT Model 4

On each trial, the Simple RT Model 4 encodes the stimulus and attempts to retrieve the answer by using the stimulus as a retrieval cue. If a chunk is retrieved, the model responds with the associated value. However, if the model fails to retrieve a chunk, it guesses the answer with equal probability of selecting a candidate answer.  

# Task 
In the paired associates task, subjects learn arbitrary letter number pairs, such as
...
In what follows, we will define a mapping between an observed response $z_i \in \{1,2 \cdots, n,\emptyset\}$ and the retrieved chunk. The retrieval set is defined as:  

$\begin{align}
R = \{\mathbf{c}_m \in M : c_m(\rm letter) = \rm z_i\} 
\end{align}$

In this particular model, the retrieval set is simplying one-to-one, meaning $|R| = 1$. As will be explained below, a retrieval error $\mathbf{c}_{m^\prime}$ could have occured, but we will treat this possibility separately as part of a mixture.
...
# ╔═╡ b7e915f0-31d9-4fa7-9744-72ccc5a82631
md"
## Conflict resolution and perceptual-motor time

As explained in earlier iterations of the model, we will fix $t_{\rm er}$ to the following value based on the default ACT-R parameters 
...
$\begin{align}
 t_\textrm{er} = \rm (cr + e) + cr +  (cr + me) + .05 = .345 
\end{align}$

"

# ╔═╡ c4dc94eb-2f6c-4c03-ba4a-47639a192da4
Markdown.parse("
## Likelihood Function

As for Simple RT Model 2 and Simple RT Model 3, the likelihood function is based on the [Lognormal Race Model](./open?path=$path_lognormal). We will define the vector of data for response and response time pairs as: \$\\mathbf{Y} = \\{\\mathbf{y}_1, \\mathbf{y}_2,  \\dots, \\mathbf{y}_{n_t}\\}\$ such that \$\\mathbf{y}_i = \\{ z_i,\\textrm{rt}_i\\}\$ is the joint response and reaction time for trial \$i\$. As previously noted, the model consists of a retrieval process and a guessing process. The PDF for the retrieval process is defined as follows:
")

# ╔═╡ 36c98cd7-79a3-461b-bff0-999ddc038c7f
md"

$\begin{align}
f_{\rm retrieve}(\rm z,\textrm{rt} \mid \Theta) 
= \sum_{\mathbf{c}_m\in R} 
g(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_m,\sigma)
\prod_{\mathbf{c}_z \in \left\{M \cup \mathbf{c}_{m^\prime} \right\}\setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_z,\sigma)\right].
\end{align}$

If a retrieval failure occurs, the model 

$\begin{align}
f_{\rm guess}(\rm z,\textrm{rt} \mid \Theta) 
=  \frac{1}{|M|} g(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_{m^\prime},\sigma)
\prod_{c_z \in M}\left[1-G(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_z,\sigma)\right].
\end{align}$

where $g$ is the Lognormal PDF, $G$ is the Lognormal CDF, and $\Theta = \{\mu_1,\mu_2,\dots \mu_n, \sigma, t_\textrm{er}\}$ a set of parameters.

The PDF of the mixture is defined as 

$\begin{align}
f(z,\textrm{rt} \mid \Theta) = f_{\textrm{retrieve}}(z,\textrm{rt} \mid \Theta)  + f_{\textrm{guess}}(z,\textrm{rt} \mid \Theta) 
\end{align}$

$\begin{align}
\mathcal{L}(\Theta; \mathbf{Y}) = \prod_{i=1}^{n_t} f(z_i,\textrm{rt}_i \mid \Theta)
\end{align}$
...
1. The model guesses if a retrieval failure occurs
2. Activation is a function of similarity between the retrieval cue and chunk
3. Each response is independent of others
4. Retrieval times follow a Lognormal Normal Race process 
"

# ╔═╡ 8287420f-f562-438d-86cf-4c36d5f95e09
md"
# Generate Data
...
- n_items: the number of items or chunks
- stimuli: a vector of integers representing stimuli
- parms: a `NamedTuple` of fixed parameters
- blc: a constant value added to activation.
- s: logistic scalar for activation noise
-  $\tau$: the retrieval threshold parameter. 

The function `simulate` creates an `ACTR` model object and then generates a simulated response for each stimulus by calling `simulate_trial`.
"

# ╔═╡ 7d0d7385-938f-4722-a5c7-1982c7e47b5c
md"

The function `simulate_trial` simulates data for a single trial. The required arguments are:

- actr: an `ACTR` model object
- n_items: the number of items stored in declarative memory
- stimulus: an integer representing a stimulus

In `simulate_trial`, a chunk is retrieved. If the chunk is empty, the model guesses from 1 to `n_items` with equal probability. An extra $.05$ seconds is added to the reaction time to account for the additional guessing process. If a chunk is retrieved, it is coded as 1 if it is correct (i.e. the response matches the stimulus). Otherwise, it is coded as 2 for incorrect. 
"

# ╔═╡ ba749357-851c-4035-8bfa-98a50a4d6a79
...
# ╔═╡ 89087eb9-b2da-419a-82b7-d74ecc9f90ac
md"
...
The code below shows the log likelihood function for Simple RT Model 1. First, we define a distribution object that holds a value for the parameter blc and a `NamedTuple` of fixed parameters called parm. The function `computeLL` function accepts the data and model parameters. `computeLL` creates an LRN distribution object that has the following fields:

-  $\mu$: a vector of activations (e.g. blc for a correct response and $\tau$ for an incorrect response)
-  $\sigma$: the standard deviation of the Lognormal distribution in log space
-  $\phi$: perceptual-motor processing time or $t_{\rm er}$

In the following loop, the log likelihood is incremented for each data point.
"
...
To ensure the PDF is working correctly, the interactive plot below superimposes the PDF over the histogram of simulated data. The orange line representing the PDF provides a good fit to the histogram, indicating the PDF and simulated data agree. 
"
...
$s \sim \rm Tnormal(.3, .5)_0^{\infty}$

$\mathbf{\mu} = [\mu_1, \mu_2,\dots, \mu_{n}, \tau]$
 
$\mathbf{y}_i \sim \textrm{LNR}(-\mathbf{\mu}, \sigma, t_{\rm er})$

In computer code, the model is specified as follows:
"

# ╔═╡ 978faac1-da3a-4a7f-a023-106b10375ed9
...
# ╔═╡ 31d1b0b2-92ae-45c5-9d32-3b9d1621c3ac
md"
## Estimate Parameters
...
manifest_format = "2.0"
project_hash = "db001ce72d1f1d2d38258b65c7b6d3087fcdc8e3"
