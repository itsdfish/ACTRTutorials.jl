...
In Unit 1, we developed several simple models of memory designed to predict which memory was retrieved. These models differed in terms of the types of errors they could produce and whether guessing occured as a result of a retrieval error. However, these models lacked predictions for reaction times. The focus of Unit 2 will be extending the models developed in Unit 1 to generate predictions for reaction times. As an overview, Unit 2 will cover the following models:

- Simple RT Model 1: the model always retrieves the correct chunk

- [Simple RT Model 2](./open?path=$path_u2_2): the model can produce a correct response by retrieving the correct chunk or commit an error of omission via a retrieval failure


- [Simple RT Model 3](./open?path=$path_u2_3): the model can produce a correct response by retrieving the correct chunk, commit an error of comission via partial matching, or commit an error of omission via a retrieval failure

- [Simple RT Model 4](./open?path=$path_u2_4): the model is a mixture of two processes—a retrieval process, or a guessing process that follows a retreival failure. Thus, only errors of comission are possible and retrieval failures are not directly observed. 

One reason to model reaction time is because reaction time distributions provide a rich source of information regarding the underlying cognitive mechanisms that generated the data. For example, reaction time may distinguish between models that predict similar choice patterns, but different processing times. From a practical perspective, including reaction times can improve the quality of the parameter estimates by including more information. Our goal in this Unit will be demonstrating techniques for developing models of reaction time. As before, we will begin with a simple model and progress to more complex models.


")

# ╔═╡ 2149ac14-baf5-490d-8321-7dcfb11a810d
md"""

## Task

As with the tasks described in Unit 1, we will use variations of a simple paired associates task throughout Unit 2. In the paired associates task, subjects learn arbitrary letter number pairs, such as
...
The model is based on the assumption that memories are highly distinguishable and have identical and stable activation, with the exception of transient noise. The model also assumes that the correct answer is always retrieved. What differs from trial to trial is the time required to retrieve and respond. 
...
On each trial $i$, a letter $v$ is presented on a screen and the model encodes the stimulus into a chunk that is stored in the imaginal buffer. Formally, we define the encoded chunk as:

$\begin{align}
\mathbf{c}_{i,\rm \textrm{imaginal}} = \{ (\rm letter, v)\}
\end{align}$

### Retrieval Request

The retrieval request on trial $i$ is defined as:

$\begin{align}
\mathbf{r}_i = \{(\textrm{cue},c_{i,\rm imaginal}(\rm letter)\} 
\end{align}$

where $c_{i,\rm imaginal}(\rm letter)$ is the letter  of the chunk in the imaginal buffer. The slot set for the retrieval request is defined as $Q_r = \rm \{letter\}$ 

### Activation

The Simple RT Model 1 is similar to the Simple Choice Model from the previous unit. As before, the activation equation is defined as:
...
In ACT-R, reaction time is the sum of many cognitive processes, which are broadly categorized as retrieval time and perceptual-motor time. In what follows, we will specify the processing times that comprise the Simple RT Model 1. 
...
The resulting distribution is called a Lognormal distribution. A random variable follows a normal distribution when it is normally distributed following a logarithmic transformation. Formally, the distribution of $t_m$ is stated as:
...
# ╔═╡ da27498f-ef7f-46b0-ab2d-5f552cc53060
Markdown.parse("
where \$\\sigma = \\frac{s\\pi}{\\sqrt{3}}\$. A more detailed explanation of the Lognormal distribution and the related Lognormal Race Model can be found [here](./open?path=$path_lognormal).
")

# ╔═╡ a61d33d3-c172-4905-b995-719c75475674
md"

### Conflict resolution and perceptual-motor time

Other processes in ACT-R contribute to the observed reaction time, including conflict resolution and perceptual-motor processes. A common simplification in applications of ACT-R involves fixing the processing time for each of these processes to the theoretically based expected value.  This is also a common practice in a large class models called sequential sampling models. This residual time is a contsant usually denoted as $t_{\textrm{er}}$. $t_{\textrm{er}}$ is typically treated as a parameters that is estimated from the data. One advantage of ACT-R, however, is that $t_{\textrm{er}}$ can be derived from the architecture. This model has three productions: encode, retrieve, and respond, each with a conflict resolution. Thus, we will fix this parameter at 
...
We define $\mathbf{Y} = \{y_1,y_2,\dots y_{n}\}$ as a set of reaction times for $n$ observations. The likelihood function is defined as:

$\begin{align}
    \mathcal{L}(\textrm{blc};\mathbf{Y}, s, \mathbf{r}_i) = \prod_{i=1}^{n} f_{\rm Lognormal}(y_i-t_{\rm er}| -\textrm{blc}, \sigma)
\end{align}$

where $f_{\rm Lognormal}$ is the PDF of the Lognormal distribution, which is defined as
...
# ╔═╡ e4824c22-8fcd-4567-9c66-5d9b6f157967
md"

# Generate Data

We will use the function below to generate data for parameter estimation. The function `simulate` accepts the following arguments:
- parms: a `NamedTuple` of fixed parameters
- blc: a constant value added to activation. The annotated code below shows how an `ACTR` object is created, a chunk is retrieved and a reaction time is computed.
"

# ╔═╡ 006440c1-377d-4450-8d7c-145bb37813ad
...
# ╔═╡ de8670fb-4ae3-424a-bb00-5a36ab309657
md"
Now that the `simulate` function has been defined, we can now generate some data. In the code block below, 50 simulated reaction times are simulated from the model
"
...
Before defining the log likelihood function `logpdf`, we will define a distribution object. The distribution object `RT` stores the parameter `blc` and a set of fixed parameters `parms`.

"

# ╔═╡ b6e0c1e2-f969-4b56-9256-c402fede8042
md"
The log likelihood function for Simple RT Model 1 is defined in the code block below. First, we define a distribution object that holds a value for the parameter blc and a `NamedTuple` of fixed parameters called parm. The function `computeLL` accepts the reaction time data (rt) and the parameters blc, ter and s as keyword arguments. The function `computeLL` also defines a Lognormal race distribution object, which reduces to a Lognormal distribution in the present case. The last line of `computeLL` computes the sum of log likelihoods for each rt. 
"

# ╔═╡ dc1c16a2-162b-4c68-9efe-02cd556f7e53
md"

## Plot PDF

In the code block below, we superimpose the density from the likelihood function over a histogram of simulated data. The resulting plot shows that the density matches the histogram quite well.
"
...
$\begin{align}
\textrm{rt} \sim \rm lognormal(-\mu, \sigma)
\end{align}$

where $\mu = $ blc is the deterministic part of activation, s is the scale parameter of a logistic distribution, and $\sigma$ is activation noise. The model is specified in Turing as follows:
"

# ╔═╡ 5dd67670-96e9-44f5-8938-2f4b2f4a7492
md"
The corresponding code for the model is defined in the cell below:
"

# ╔═╡ bd23fa52-9eea-4b48-aa79-ba220bc03e64
@model model(data, parms) = begin
    blc ~ Normal(1.5, 0.5)
    data ~ RT(blc, parms)
end

# ╔═╡ dd844b7a-7feb-4f82-9e75-9b2e6e5bcb25
md"
## Estimate Parameters
...
A summary of the parameter estimates can be found in the output above. Diagnostics for the MCMC chain are ploted in the code block below. The trace plot (top panel) does not show any irregularities and the autocorrelation plot (middle panel) shows a quick drop in autocorrelation with increasing lag. In the third panel, the density plot shows that the posterior distribution of blc is centered near the true value of 1.5.
"
...
In the code block below, we create a posterior predictive distribution for the rt distribution. The posterior predictive distribution is a mixture of Lognormal distributions and shows the characteristic skew commonly found in reaction time distributions.
"
...
The next plot illustrates that the predictive posterior distribution is a mixture of Lognormal distribution in which each component is weighted by the posterior probability of its corresponding blc parameter.
"
...
PlutoUI = "~0.7.59"
Revise = "~3.5.15"
SequentialSamplingModels = "~0.11.5"
StatsPlots = "~0.15.7"
Turing = "~0.33.1"
"""
...
manifest_format = "2.0"
project_hash = "2c16d0d1bc4782962ec13de26fd4422a3d416e8a"
...
deps = ["Distributions", "DynamicPPL", "FunctionZeros", "HCubature", "Interpolations", "KernelDensity", "LinearAlgebra", "PrettyTables", "Random", "SpecialFunctions", "Statistics", "StatsAPI", "StatsBase"]
git-tree-sha1 = "3b2236d2fc4fdf3892df269001fce42b9cc66de1"
uuid = "0e71a2a6-2b30-4447-8742-d083a85e82d1"
version = "0.11.5"
weakdeps = ["Plots", "Turing"]

    [deps.SequentialSamplingModels.extensions]
    PlotsExt = "Plots"
    TuringExt = "Turing"
