...
# ╔═╡ 70a2bed1-fba7-4f60-91c2-b619cb2e9c2f
md"""
...
On each trial, the Simple RT Model 2 encodes the letter on the screen and attempts to retrieve the associated number. We assume that memories are highly distinguishable and have identical and stable activation, with the exception of transient noise. If a chunk is retrieved, the model responds with the associated number by key press. If the model fails to retrieve a chunk, it responds with the key corresponding to "I don't know".   
...
```math
\begin{align}
\mathbf{c}_{i,\rm imaginal} = \{ (\rm letter, v)\}
\end{align}
```
...
As the name implies, the model the model is rather simple. A retrieval request $\mathbf{r}_i$ is submitted to the declarative memory buffer. The correct memory is retrieved if its activation is above a retrieval threshold, $\tau$. Otherwise a memory retrieval failure occurs, prompting the model to omit a response, or equivalently, indicate "I don't know" by pressing the designated key.

### Retrieval Set

In what follows, we will define mapping representing all chunks that could have been retrieved and used to generate the observed response on trial $i$. Define $z_i$ as the response on trial $i$ where $z_i \in \{1,2,\dots, n, \emptyset \}$, and $\emptyset$ indicates a retrieval failure. 

$\begin{align}
R = \{\mathbf{c}_m \in M : c_m(\textrm{number}) =  z_i\}
\end{align}$

In this particular model, the mapping is simplying one-to-one, meaning $|R| = 1$. 
...
# ╔═╡ 5a7336b1-d6e5-4587-82e0-84882f754e35
Markdown.parse("
## Conflict resolution and perceptual-motor time

As explained in earlier iterations of the [model](./open?path=$path_u2_1), we will fix \$t_{\\rm er}\$ to the following value based on the default ACT-R parameters as follows:
...
# ╔═╡ b7405357-48fb-4d5c-9c9d-705296a00c31
md"
...
# ╔═╡ 115b8538-afd9-4dcd-a7cb-b2f37aef6845
Markdown.parse("
## Likelihood Function

The likelihood function for Simple RT Model 2 is based on the Lognormal Race model (LNR). The LNR describes how ACT-R's memory retrieval processes can be interpreted as a race among competing evidence accumulation processes, where the winner is retrieved. A more detailed description of the LNR can be found [here](./open?path=$path_lognormal). The joint density of resp at time rt is defined as the following PDF:
")

# ╔═╡ cdd21c9e-2462-4a52-94ac-2b74e9d5d3b2
md"


$\begin{equation}
f_{\rm LNR}(\rm z,rt \mid \Theta) = \sum_{\mathbf{c}_m \in R} g(\textrm{rt}-t_{\textrm{er}} \mid - \mu_m, \sigma) \prod_{k \in \{M \cup \mathbf{c}_{m^\prime}\} \setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-t_{\textrm{er}}|-\mu_k,\sigma)\right]
\end{equation}$

where $g$ is the Lognormal PDF, $G$ is the Lognormal CDF, and $\Theta = \{\mu_1,\mu_2,\dots \mu_n, \sigma, t_\textrm{er}\}$ is a set of parameters. We will define the vector of  data for response and response time pairs as: $\mathbf{Y} = \{\mathbf{y}_1, \mathbf{y}_2, \dots, \mathbf{y}_{n_t}\}$, such that $\mathbf{y}_i = \{\rm z_i,rt_i\}$ is the joint response and reaction time for trial $i$. The likelihood function is given as:

$\begin{align}
\mathcal{L}(\Theta; \mathbf{Y}) = \prod_{i=1}^{n_t} f_{\rm LNR}(\rm z_i,rt_i; \Theta)
\end{align}$
"

# ╔═╡ ed30a406-19b3-4fce-9ce3-17b3798c1334
md"
# Generate Data
...
`simulate` creates an `ACTR` model object, retrieves a chunk, and codes the response as 1 as correct and 2 as a retrieval failure. The annotated code below shows how an ACTR object is created, a chunk is retrieved and a reaction time is computed. 
"

# ╔═╡ c6deb787-3612-417b-8a73-6f7e447cd917
...
md"
As shown in the code block below, data for each trial is a `NamedTuple` containing a response (resp: 1 if correct, 2 otherwise) and a reaction time.
"

# ╔═╡ d4d2f47c-78ab-460d-a28a-c996cb5afd94
data[1]

# ╔═╡ 1e7f9fb4-3f13-4483-b9cb-6167c51690d5
md"
...
# ╔═╡ 60dadcfe-b1ed-44ab-9473-4afca9f11a05
md"
The slider located below allows you to adjust the value of blc. As blc increases, the density for correct RTs increases relative to incorrect RTs. In addition, the mean RT decreases for both correct and incorrect responses and more density is placed on fast responses.
"
...
$\begin{align}
\textrm{blc} \sim  \rm normal(1.25, 0.5)
\end{align}$
...
# ╔═╡ 2ee61750-e2b4-412b-a03f-5c3cf3b00896
...
# ╔═╡ 77538e24-e372-4758-80b4-7a5197fb4a07
md"
## Estimate Parameters
