...
The goal of this tutorial is to develop a memory based model of addition that predicts choices and reaction times. This model extends the [Siegler addition model](../../Unit3/Siegler_Choice/Siegler_Choice_Model.ipynb) from the Unit 3 so that it generates reaction time predictions. The model retains the same strategy for solving the addition problems: rather counting to the [solution](../../Unit4/Addition/Addition_Model.ipynb), it assumes that the solutions are retrieved from memory. In what follows, we will review the task and then present the model.
"
...
# ╔═╡ c1113eed-7987-4e1d-af5d-0856a61ab269
md"
## Declarative memory
...
# ╔═╡ 5d34290c-43b9-4a0d-a6f7-222599d91fe7
md"
...
# ╔═╡ f6dbdadb-9ffb-4558-82fb-ab05b5898f33
md"
## Activation
...
The default mismatch penalty function in ACTRModels.jl simply counts the number of mismatches and weights the count by $\delta$, the mismatch penalty parameter. We will pass the custom function defined below.
...
The response is determined by the value associated with the "sum" slot of the retriceved chunk: $r_i = c(\textrm{sum})$. Due to the partial matching mechanism, a given response could be associated with one of several possible chunks. For example, the response $r_i = 4$ could have resulted from retrieving any chunk that represents the following addition facts:
...
In order to account for this in the likelihood function, we define the set $R_i$, which consists of all possible chunks associated with response $r_i$. Formally, this is defined as:

$\begin{align*}
R_{\rm i} &= \{\mathbf{c}_m \in M : c_m(\rm sum) = r_i \} &\textrm{ fo } r_i\ne \emptyset\\
R_{\rm i} &= \{\mathbf{c}_{m^\prime}\}  &\textrm{ for } r_i =\emptyset.
\end{align*}$
...
# ╔═╡ a5a1588b-c108-48bb-8986-ab264f9d69b3
md"
## Likelihood Function

Let $\mathbf{Y} = \{\mathbf{y}_1, \mathbf{y}_2, \dots, \mathbf{y}_{n}\}$ be joint choice-response time pairs, such that $\mathbf{y}_i = \{\rm r_i,rt_i\}$ contains the response and reaction time for trial $i$, respectively. 
Recall that the likelihood function for retrieving chunk $m$ is is based on the [Log Normal Race Model](../../../Background_Tutorials/Lognormal_Race_Process.ipynb):

$\begin{equation}
\mathcal{L}_{\rm LNR}(\Theta; \mathbf{c}_m, \textrm{rt})
= g(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_m,\sigma)
\prod_{\{\mathbf{c}_z \in M \cup \mathbf{c}_{m^\prime} \} \setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_z,\sigma)\right],
\end{equation}$


where $g$ is the Lognormal PDF, $G$ is the Lognormal CDF, and $\Theta = \{\mu_1,\mu_2,\dots \mu_N, \sigma, t_\textrm{er}\}$ is a set of parameters.The likelihood function for a single trial of the Siegler model is given as:

$\begin{equation}
\mathcal{L}(\Theta; r,\textrm{rt})
= \sum_{\mathbf{c}_m \in R_{i}} f_{\rm LNR}(\mathbf{c}_m,\textrm{rt};\Theta),
\end{equation}$

Across all trials, the likelihood function is defined as:

$\begin{align}
\mathcal{L}(\Theta; \mathbf{Y}) = \Pi_{i=1}^{N} \mathcal{L}(\Theta; r_i,\textrm{rt}_i)
\end{align}$
"

# ╔═╡ 901ae2d6-f7b6-46da-89fa-40cf1039eb3d
md"""

### Example

As an example, suppose that a subject is given the question "What is 1 + 2?" and responds with the answer "3". The set of candidate chunks in this case is the following: 

-  $\mathbf{c}_1 = \{(\textrm{addend1}, 1), (\textrm{addend2},2), (\textrm{sum},3) \}$
-  $\mathbf{c}_2 = \{(\textrm{addend1}, 0), (\textrm{addend2},3), (\textrm{sum},3) \}$
-  $\mathbf{c}_3 = \{(\textrm{addend1}, 2), (\textrm{addend2},1), (\textrm{sum},3) \}$
-  $\mathbf{c}_4 = \{(\textrm{addend1}, 3), (\textrm{addend2},0), (\textrm{sum},3) \}$
...
cm"""
According to the model, any of these four chunks could have been retrieved. Since we do not know which chunk was retrieved, we must compute the likelihood over all four chunks. The figure below provides an illustration of this process. The first row shows each of the four potential winning chunks with the slot-pairs displayed in the titles. The subplots in the first row are ordered from most likely (i.e. an exact match) to least likely (i.e. the largest possible mismatch). The dashed vetical line represents the winning time of ``t = `` $(win_time) seconds. Within each column, the second, third and fourth rows show the fastest losing chunks. Be aware that in actuality there are ``34`` competing chunks for each of the four possible winning chunks. The other ``30`` were omitted for ease of visual presentation.

For each of these chunks, we know that they finished sometime *after* ``t =`` $(win_time) seconds. However, we do not know exactly when they finished. For this reason, we will represent the possible finishing times for the losing chunks with the blue area under the curve, as opposed to a vetical line representing a density. 


The sliders below allow you to change the the time of the winning chunk and the mismatch penalty parameter. The likelihood of the response is provided below the plots. As you can see, a value of `win_time` of approximately ``.3`` has the highest likelihood. Values lower or higher than ``.3`` are progressively less likely. Decreasing `δ₁` will shift more density towards fast finish times for chunks that do not match the retrieval request perfectly. 
"""
...
# ╔═╡ 8f861626-5749-4e26-85ba-893be556dd13
md"
...
The function `computeLL` operates in a similar manner to the function `simulate`, which generates data from the model. The function `computeLL` begins by initializing the declarative memory and ACT-R objects. The for loop iterates through the data and breaks down the likelihood in two cases. If a retrieval failure occurs, as indexed by -100, the log likelihood is computed using a simple Lognormal race model. However, all other responses are a mixture of Lognormal races across the different chunks in $R_{\textrm{resp}}$. This requires computing the log likelihood of each mixture component and finding the marginal log likelihood with the function `logsumexp`, which provides a numerically stable way to sum logarithms. 
"
...
manifest_format = "2.0"
project_hash = "557eaf6f62918f1e61f9c9242358f98924a6814e"
