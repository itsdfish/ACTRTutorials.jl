...
function mixture_parms(max_cycles; γ, mt = 0.06, ρ = 2 / 3)
    ...
    sigma = similar(mu)
    cycles = [1:max_cycles;]
    @. mu[:, 1] = cycles * γ + mt + 0.5 * γ
    @. mu[:, 2] = cycles * γ + 0.085 + mt + 0.5 * γ
    @. sigma[:, 1] = 1.05 * sqrt(
        (1 / 12) * (cycles * (γ * ρ) .^ 2 + (mt * ρ)^2)) + 0.1 * γ
    @. sigma[:, 2] =
        1.05 * sqrt.(
...
"""
LogLikelihood(rt::Float64; υ, τ, λ, γ, σ = 0.45345, max_cycles = 15) =
    LogLikelihood([rt]; υ, τ, λ, γ, σ, max_cycles)

function LogLikelihood(RTs::Vector{Float64}; υ, τ, λ, γ, σ = 0.45345, max_cycles = 15)
    LL = 0.0
...
function likelihood_trial(rt, mus, sigmas, weights, max_cycles)
    ...
    for att = 1:2
        for n_cycles = 1:max_cycles
            L +=
                pdf(Normal(mus[n_cycles, att], sigmas[n_cycles, att]), rt) *
                weights[n_cycles, att]
        end
    end
    return L
end
...
function simulate(υ, τ, λ, γ, trials)
    egs = 0.45345 ##utility noise free [.45345]
