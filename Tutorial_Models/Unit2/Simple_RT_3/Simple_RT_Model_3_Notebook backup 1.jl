...
# ╔═╡ ca390752-5d70-40c0-8ec3-143676ac0114
begin
...
# ╔═╡ 19f4fcff-81c5-4b0d-b388-6469d4289d6b
begin
...
# ╔═╡ b60e2415-8e39-417f-b7af-9e6af4204cd4
md"""
# Introduction

In the previous [tutorial](../../../Tutorial_Models/Unit2/Simple_RT_2/Simple_RT_Model_2.ipynb), we developed a simple model of cued recall that produced one of two responses: a correct answer or an incorrect answer due to a retrieval failure. The goal of the present tutorial is to extend [Retrieval Model 2](../../../Tutorial_Models/Unit2/Simple_RT_2/Simple_RT_Model_2.ipynb) to produce errors of comission due to confusions between declarative facts. This model will use partial matching to produce errors of comission, much like [Simple Retrieval Model 2](../../../Tutorial_Models/Unit1/Simple_Retrieval_2/Simple_Retrieval_Model_2.ipynb).

## Task

We will use the paired associatesl task used in [Retrieval Model 2](../../../Tutorial_Models/Unit2/Simple_RT_2/Simple_RT_Model_2.ipynb). 

# Simple RT Model 3

On each trial, the Simple RT Model 3 encodes a stimulus located on the screen and attempts to retrieve the answer using the stimulus as a cue. If a chunk is retrieved, the model responds with the value of the value slot. If the model fails to retrieve a chunk, it responds with the key corresponding to "I don't know".   

## Declarative memory

Let $M$ denote the set of  $n$ chunks in declarative memory, one chunk for each stimulus used in the experiment. Each chunk consists of the following set of slots: $Q= \rm \{letter,value\}$. 
...
$\begin{align}
\mathbf{r}_i = \{(\textrm{letter},c_{i,\textrm{imaginal}}(\rm letter)\} 
\end{align}$
...
In what follows, we will define a mapping between an observed response $z_i \in \{1,2 \cdots, n,\emptyset\}$. The retrieval set is defined as:  

$\begin{align}
R = \{\mathbf{c}_m \in M : c_m(\rm letter) = \rm z_i\}
\end{align}$

In this particular model, the retrieval set is simplying one-to-one, meaning $|R| = 1$. 
...
# ╔═╡ 61ce1ea8-b6d1-45a0-b225-6f272fc47fd9
md"
...
$t_\textrm{er} = \rm (cr + e) + cr +  (cr + me) = .295$ seconds
...
The likelihood function for Simple RT Model 3 is based on the [Log Normal Race Model](../../../Background_Tutorials/Lognormal_Race_Process.ipynb). The joint likelihood of resp at time rt is defined as:

$\begin{equation}\
f_{\rm LNR}(\rm z,rt;\Theta) = \sum_{\mathbf{c}_m \in R} g(\textrm{rt}-t_{\textrm{er}} \mid - \mu_m, \sigma) \prod_{k \in \{ M \cup \mathbf{c}_{m^\prime}\} \setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-t_{\textrm{er}}|-\mu_k,\sigma)\right]
\end{equation}$

where $g$ is the Lognormal PDF, $G$ is the Lognormal CDF, and $\Theta = \{\mu_1,\mu_2,\dots \mu_n, \sigma, t_\textrm{er}\}$ is a set of parameters. We will define the vector of  data for response and response time pairs as: $\mathbf{Y} = \{\mathbf{y}_1, \mathbf{y}_2, \dots, \mathbf{y}_{n_t}\}$, such that $\mathbf{y}_i = \{\rm z_i,\textrm{rt}_i\}$ is the joint response and reaction time for trial $i$. The likelihood function is given as:

$\begin{align}
\mathcal{L}(\Theta ; \mathbf{Y}) = \prod_{i=1}^{n_t} f_{\rm LNR}(\rm z_i,\textrm{rt}_i \mid \Theta)
\end{align}$
...
# ╔═╡ af1c8c1b-3a95-4dc3-a2bb-767ce290b881
md"
# Generate Data
...
# ╔═╡ 5ba20aac-1c32-4c3c-b938-7c2e238ad289
md"
`simulate_trial` simulates a single trial of the cued recall experiment and accepts the following arguments:

- actr: an actr model object
- stimulus: a number representing a cue
- n_items: the number of items or chunks

`simulate_trial` works by retrieving a chunk and coding the result into one of three categories:

  1. code as 1 if the response is correct
  2. code the response as 2 if incorrect
  3. code the response as `n_items + 1` if retrieval failure which

This simplified response coding can be used because the $n-1$ incorrect responses are indistinguishable from each other because each has a mean activation of $\mu_m = \textrm{blc} - \delta$
"

# ╔═╡ 4c17875a-6084-46f4-9b46-065448763b46
...
# ╔═╡ c208246a-6cbc-46b0-9f4c-6838725cf1c5
md"
Reveal the cell below to see helper functions.
"

# ╔═╡ 7cfb49fe-7cdf-4480-b1cf-603a95871e97
begin
	"""
		sample_stimuli(n, m)
...
# ╔═╡ f9259bf4-e2a3-4db2-9d37-21b8a6f332ed
...
# ╔═╡ 461955c7-6971-4f4f-9eb6-eb495824367b
begin
	# Number of trials
	n_trials = 50
	...
	# Generate Data
	data = simulate(n_items, stimuli, parms; δ, τ)
	data[1]
end

# ╔═╡ 011bfd48-8520-4e0b-8f79-ca5c8adf9e2b
md"
## Define Log Likelihood Function

The code below shows the log likelihood function for Simple RT Model 3. First, we define a distribution object that holds a value for the parameter blc and a `NamedTuple` of fixed parameters called parm. The function `computeLL` function accepts the data and model parameters. `computeLL` creates an LRN distribution object that has the following fields:
...
# ╔═╡ 4d37c351-bfd1-4d35-acd5-83089c0c0099
begin
...
let 
	...
	ter = (0.05 + 0.085 + 0.05) + (0.05 + 0.06)
	# Fixed Parameters
	parms = (noise = true,ter = ter, s=0.3, blc=_blc, mmp=true)
	# Generate Data
	ex_stimuli = fill(1, n_trials)
	sim_data = simulate(n_items, ex_stimuli, parms; δ, τ)
	# correct rts
	...
	density_correct = map(x-> computeLL(n_items, parms, [(resp=1, stimulus=1,rt=x)]; δ, τ) |> exp, times)
	# plot correct density
	p = plot(layout=(2,1), leg=false, xlabel="RT (seconds)", ylabel="Density", size=(600,600), ylims=(0,4))
	...
	density_incorrect = map(x-> computeLL(n_items, parms, [(resp=2,stimulus=1,rt=x)]; δ, τ) |> exp, times)
...
# ╔═╡ 52fd1ed7-9198-4e61-8426-925a2aa8089c
md"
## Define Model
...
$\begin{align}
\boldsymbol{y_i} \sim \textrm{LNR}(\Theta)
\end{align}$

In computer code, the model is specified as follows:"

# ╔═╡ cc179875-5869-4e29-9308-0e86ea6646a9
@model model(data, parms, n_items) = begin
    δ ~ Normal(1.0, 0.5)
    τ ~ Normal(0.5, 0.5)
    data ~ RT(δ, τ, n_items, parms)
end

# ╔═╡ 087a1112-7fb9-4f9c-bec1-ca8e6831d1ce
md"
## Estimate Parameters
...
# ╔═╡ 3101b038-76ae-4ae5-b1bb-11bf16452176
begin
...
# ╔═╡ be0abcfb-d7d1-46d6-9a2f-a565dd5ec5a1
md"
## Results
...
# ╔═╡ 62da617a-ff16-4e37-bde9-38b5ab316179
let
	ch = group(chain, :δ)
	font_size = 12
...
# ╔═╡ 81a47084-17c5-4274-a534-000bc27cbd2f
begin
	font_size = 12
	ch = group(chain, :τ)
	p1 = plot(ch, xaxis=font(font_size), yaxis=font(font_size), seriestype=(:traceplot),
...
# ╔═╡ c2d72edc-f353-4039-8f28-8002386f82a7
md"
### Posterior Predictive Distribution

The code block below plots the posterior predictive distributions for correct rts and incorrect rts. As expected, the density for incorrect rts is lower than correct rts, which reflects the fact that incorrect responses are less probable. In addition, incorrect RTs are about .10 seconds slower on average than correct RTs. As before, there are $n-1$ identical incorrect response distributions. Only one is shown for ease of visual presentation.
"

# ╔═╡ 77fa8941-589a-4a89-9aba-feaea00a02a5
let 
	font_size = 12
...
manifest_format = "2.0"
project_hash = "249b5f8c9257b1b483a2431103f37b7505538c85"
