...
In this tutorial, we will develop a the fan model described in the standard ACT-R tutorial. The fan model uses spreading activation to account for the classic fan effect, according to which the time to retrieve a fact increases with the number of assocations it has. Compared to previous tutorials, the likelihood function features a more complex response mapping and activation function.
...
The time course of cognitive processing is illustrated in the Figure below for a typical trial. The model makes three simplifying assumptions about the time course of cognition: (1) the  processing time of each module is deterministic except for declarative memory, (2) a production rule is selected only if the buffer contents match the conditions exactly, resulting in a deterministic chain of cognitive processes, and (3) as evident in the Figure below, the model produces a mixture of serial and parallel processing. In particular, the imaginal buffer operates in parallel with the procedural and visual modules while the remaining cognitive processes operate in serial fashion.

On each trial, the model proceeds through a series of production cycles in which the procedural module selects a production rule during conflict resolution  (e.g. the top row of the Figure below) and then executes the actions in the selected production rule, which ultimately culminates in a response. When the person-place pair is presented, the model begins by finding the "person" word and creates a chunk in the imaginal buffer to store the person-place pair. While the imaginal module creates a new chunk, the model encodes the "person" word into the visual buffer and then retrieves semantic information associated with the word. After waiting for the completion of the imaginal module, the model transfers the "person" word from the visual buffer to the imaginal buffer. The model repeats this entire sequence of events for the "place" word, except for creating a new chunk in the imaginal buffer. Next, the model attempts to retrieve the person-place fact from declarative memory by using the person and place values of the imaginal chunk as a retrieval cue. After the retrieval attempt, the model selects and executes one of three production rules that maps the result of the memory retrieval to the corresponding key.    
"""
...
$\begin{align}
\mathbf{c}_{s,\textrm{imaginal}} = \{ (\textrm{person}, v_{s,1}), (\textrm{place}, v_{s,2})\}
\end{align}$
...
In the fan model, one of two production rules that issues a retrieval request is selected randomly with equal probability. One production rule uses the person slot-value pair as a retrieval request, while the other uses the place slot-value pair as a retrieval request. Accordingly, we define the retrieval request in terms of the slot $q$ on which the request is based:

$\begin{align}
\mathbf{r_q} = \{(q,c_{s,\rm imaginal}(\rm q)\} 
\end{align}$
...
Spreading activation is the mechanism responsible for producing the fan effect. As the name implies, spreading activation spreads or distributes additional activation to chunks whose slot values match the slot values of chunks contained in any of the buffers. When activation spreads across many associations, total activation increases, but by a smaller amount, resulting in relatively slower reaction times and, by extension, the fan effect. The intuition behind spreading activation is as follows: the probability that a chunk will be used depends on context---which, in this case, is the stimulus encoded into the imaginal buffer. If the fan value is large, then any given matching chunk has a diminished probability of being used within the context. In other words, the context is less discriminative when the fan value is high.

The figure produced in the code block below illustrates the relationship between fan and activation for different values of the maximum association parameter. Each value of the maximum association parameter is represented by a different colored line, where orange corresponds to higher values.
"""

# ╔═╡ 00fa3233-5b41-4cff-982b-cb5fa8b6c1c1
md"

Reveal the three cells below to see code for generating stimuli, generating chunks, and other helper functions
"
...
where $b$ is a buffer in buffer set $B$, $k$ is a slot of the chunk in buffer $b$ and $Q_b$ is the set of all slots in the chunk in buffer $b$, $W_{b,k}$ is the association weight for slot value $k$ of the chunk in buffer $b$, which is defined as $\frac{1}{|Q_b|}$ by default, and  $S_{m,k}$ is the strength of association for slot value $k$ of chunk $m$. In general, activation can spread from any number of buffers in $B$ containing a chunk. In the present model, the set of buffers is defined as $B=\{\rm imaginal\}$ because activation spreads only from the imaginal buffer. Strength of association is defined as:
...
$\begin{align}
H_{\rm yes} &= \{\mathbf{c}_m \in R_{q}: \forall q \in Q c_m(q) = p(q) \}\\
H_{\rm no} &= \{\mathbf{c}_m \in R_{q}: \exists q \in Q \textrm{ s.t. } c_m(q) = p(q) \} \cup \mathbf{c}_{m^{\prime}} = \{R_q \setminus H_{\rm yes} \} \cup \mathbf{c}_{m}^{\prime}\\
\end{align}$

where $R_{q} = \{\mathbf{c}_k \in M : \forall q \in Q_r c_k(q) = r_q(q) \}$ is the set of chunks that match the retrieval request $\mathbf{r}_q$. On target trials, set $H_{\rm yes}$ only the chunk whose slots match the stimulus. Otherwise, $H_{\rm yes} =  \emptyset$. $H_{\rm no}$ is the complimentary set containing all elements in $R_q$ not in $H_{\rm yes}$ plus the retrieval failure $\mathbf{c}_{m\prime}$
"""

# ╔═╡ 8ed53083-0787-4f1b-adac-27c66187d890
md"""
...
The likelihood function for Fan Model 1 has two functions based on the Lognormal Race model: one function for "yes" responses, and different function for "no" responses. The likelihood of responding "yes" in rt seconds is:
 
$\begin{align}
f_{\textrm{yes}}(\textrm{rt}) 
= \sum_{ \mathbf{c}_m \in H_{\rm yes}}\frac{1}{2}\sum_{q\in Q} 
g(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_m,\sigma)
\prod_{ \mathbf{c}_z\in \left\{R_{q}\cup \mathbf{c}_{m^\prime} \right\}\setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_z,\sigma)\right].
\end{align}$
...
= \sum_{\mathbf{c}_m \in H_{\rm no}}\frac{1}{2}\sum_{q\in Q} 
g(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_m,\sigma)
\prod_{\mathbf{c}_z \in \left\{R_{q}\cup \mathbf{c}_{m^\prime} \right\}\setminus \mathbf{c}_m}\left[1-G(\textrm{rt}-\textrm{t}_{\textrm{er}}|-\mu_z,\sigma)\right].
\end{align}$
 
In both functions, the outer summation marginalizes over the set of chunks $H_{\rm yes}$ or $H_{\rm no}$ that may have lead to the observed response. The inner summation represents the retrieval request process, which is an equal probability mixture of retrieval cues (person or place) used to request a chunk from declarative memory.

The following summarizes the assumptions of the model:


1. The model randomly selects between using the person or place slot as a retrieval request
...
# ╔═╡ 8c68be3a-d9c2-407b-86c8-b3264d23eaf3
md"""
# Generate Data
...
# ╔═╡ 3467846d-dde7-469e-8a60-d8a7d243c733
md"

In the code block below, we will generate 5 blocks of simulated data, producing a total of 90 trials. Each simulated trial is a `NamedTuple` containing the person and place slot-value pairs, the fan values, the reaction time and the response (:yes or :no).
"
...
# ╔═╡ e3c1c725-17c2-47c4-b24c-819cb09775c0
md"""
## Plot PDF
...
$y_i \sim \textrm{Fan}(\mathbf{\mu})$
 

In computer code, the model is specified as follows:
"

# ╔═╡ a173aa68-670f-41db-be4f-21fe8965788b
...
The grid plot below shows the mean posterior predictive reaction time for "yes" responses on target, paneled by fan condition. The means from the original simulated data are represented as a vertical red line as a reference point. As expected, the predicted distributions increase in an additive fashion with the fan value, and the predictions encompass the means of the original data.
"""
...
manifest_format = "2.0"
project_hash = "b8c6ec8ed1521dc72f7bbecf33c9bc30001a0a71"
