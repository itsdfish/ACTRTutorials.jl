...
In this tutorial, you will learn how to develop a simplified model of the psychomotor vigilance test (PVT; Gunzelmann, Gross, Gluck, & Dinges, 2009). The PVT is a reaction time task that is used to study and measure performance decrements associated with time on task, vigilance, and sleep deprivation. As you will learn shortly, the model of the PVT is simple in terms of production rules: there is one production rule to encode the stimulus, and one production rule to respond. However, the model has a rich statistical structure, characterized by a complex mixture of latent processes and dynamicism. One important lesson worth noting is that the number of production rules is not a reliable proxy for the statistical complexity of a model. As the PVT model will illustrate, what matters more is dynamicism and latent states and processes.  


# Psychomotor Vigilance Test

On each trial of the Psychomotor Vegilance Test (PVT) a millisecond counter appears after a uniformly distributed delay of 2-10 seconds (Dinges & Powell, 1985). The participant must respond by keystroke as quickly as possible. Once the participant responds, the millisecond clock stops to provide feedback. A new trial begins after the feedback disappears. A common finding is that as a person becomes fatigued, the reaction time distribution changes is three ways: an increase of false starts---defined as responding prior to stimulus onset, an increase in the mean and an increase in the variance. 
...
Unlike many other models in the tutorial, the PVT model relies heavily on procedural memory rather than declarative memory. The full PVT model consists of three production rules: wait, attend, and respond. The wait production rule fires while waiting for the stimulus to appear and is important for producing false starts because it supresses the attend and respond production rules. The attend production rule is most likely to be selected immediately following stimulus onset, prompting a shift in visual attention to encode the stimulus. Finally, the respond production rule is most likely to be selected after the stimulus has been encoded. Upon firing, the respond production rule issues a request to the motor module to press a key. 

For simplicity, we will omit the wait production rule and instead focus on the post-stimulus processes---namely attending and responding. Although this may sound uninteresting at first glance, the fatigue mechansism produces dynamics that are interesting from a statistical point of view. As will be explained in more detail below, the microlapse is an important component of the fatigue mechanism. A microlapse is a brief period of inaction which occurs when no production rule has a utility value that exceeds a utility threshold. In addition to causing a brief period of inaction, this causes a proportional decrease in utility values, which increases the chance of a  microlapse on subsequent production cycles. This cascading effect of microlapses is responsible for introducing additional skew into the distribution.
...
In the PVT model, each production rule is assigned a utility value that represents its ability to accomplish a goal which is based on the match between the production rule conditions and the state of the buffers. A production rule is selected if its noisy utility value is the highest among candidates and exceeds a utility threshold. The utility matrix below shows how the utility value is determined by the state and count of micrlopases, $n$. If the model has not attended to the stimulus, the attend production rule has a utilty value of $U(n)$ and the respond production rule has a utility value of 0. Under these conditions the attend production rule is more likely to be selected because $U(n) \geq 0$. If the model has attended to the stimulus, then the only eligible production rule is `respond`, which has utility $U(n)$.

|       | Production Rule |        |         |
|-------|-----------------|--------|---------|
|       |                 | attend | respond |
| State | not attended    | U(n)   | 0       |
|       | attended        |        | U(n)    |


More formally, we define the utility of production rule $p$ in state $s$ as:


$\begin{align}
    U_{p,s}(n) = \lambda^n(\upsilon-\textrm{mp}_{p,s}) + \epsilon
\end{align}$

where $\lambda = [0,1]$ is the fatigue decrement rate parameter, $\upsilon$ is the base utility parameter, mp$_{p,s}$ is the mismatch penalty and $\epsilon \sim \textrm{Logistic}(0, \kappa)$ with $\kappa$ being the logistic scale parameter. 

The binary mismatch penalty function is defined in terms of $\upsilon$:


$\begin{align}
\textrm{mp}_{p,s}  = \begin{cases}
    0 & \text{if } \textrm{matches} \\
    \upsilon & \text{otherwise} \\
  \end{cases}\
\end{align}$

Thus, the utility is zero in the case of a mismatch.


Expected utility is defined as

$\begin{align}
\mu_{p,s}(n) =  E\left[U_{p,s}(n) \right] =  \lambda^n(\upsilon-\textrm{mp}_{p,s})
\end{align}$

### Production Rule  Selection 

A production rule is selected if its utility values is the maximum among eligible production rules and exceeds the utility threshold $\tau$. This can be formally stated as follows:

$\begin{align}
p_{\textrm{max}} = \underset{p \in P }{\operatorname{argmax} \{ U_{p,s}(n): U_{p,s}(n) > \tau\}}
\end{align}$

where $P$ is an index set of eligible production rules.

### Selection Probability

The probability of selecting production $p$ in state $s$ is approximated with the soft max rule:

$\begin{align}
         \textrm{Pr}( p \mid s) = \frac{ e^{\frac{\mu_{p,s}(n)}{s}}}{\sum_{p \in P} e^{\frac{\mu_{p,s}(n)}{s}} + e^{\frac{\tau}{s}}}
\end{align}$

"

# ╔═╡ 2c8b6331-2faa-4895-a61f-04a42952871d
md"

## Likelihood Function

In this section, we will define the likelihood function for the PVT model. Because the model is a mixture of distributions based on microlapses, we will develop equations that characterize the processing times of mixture components and the mixture weights. 

### Mixtures

As a result of the microlapse mechanism, the PVT model does not fire a deterministic sequence of production rules---namely, attend followed by respond. Instead, there is a variable number of microlapses $n$ which occur between the first production cycle and the last production cycle ending in a response. Consider a few possible sequences of events:

1. respond
2. attend, respond
3. microlapse, microlapse, attend, microlapse, respond
4. microlapse, respond

This set of possible events illustrates that the model can respond in one of two basic ways: without attending to the stimulus (i.e. 1 and 4) or by attending to the stimulus (i.e. 2 and 3). Within each of these two basic ways of reaching a response, there is a variable number of microlapses, thus producing a mixture distribution. The primary challenge we must overcome in developing a likelihood function is enumerating the mixture components. 

Before enumerating all of the mixture components, let's look at some of the possible patters that we could observe. The function `show_cycles` generates all possible sequences of processes for a given number of production cycles. Before using `show_cyclces`, we will load the required packages and code:
"

# ╔═╡ ee7e07f9-5780-4ef4-808e-fa14af6fe026
md"
In the examples below, `CR` indicates conflict resolution, `A` indicates attending, and `R` indicates responding. The number between the parentheses indicates the number of microlapses up to that point. In each example, there are three patterns. First, given $n_c$ production cycles, there is one way to respond without attending and $n_c-1$ ways to attend and respond. Second, there are $n_c$ production cycles, there are $n_c + 1$ events for an non-attentive response ($n_c$ production cycles + respond), and $n_c + 2$ events for an attentive response ($n_c$ production cycles + attend + respond). Third, the number of microlapses $n$ increases when a conflict resolution is followed by another conflict resolution. We will capture each of these patterns below. Note that you can click the ▶ to the left of the output to expand.
"

# ╔═╡ deb1ec0d-0320-4db6-ab00-1fd1beeedf18
md"
All of the permutations for 6 cycles is given below:
"

# ╔═╡ b79771fd-68e6-412b-bb9e-7573761ac063
md"
The function `show_cycles` can be revealed below
"

# ╔═╡ 1c2d18ca-e0d7-4033-b592-2839c20640bc
md"
#### Processing Time Distributions

In ACT-R, processing time for attention, motor execution and conflict resolution follows a Uniform distribution with two parameters corresponding to minimum and maximum, denoted $u_\text{min}$ and $u_\text{max}$, respectively. The sum of Uniform random variables does not have a simple solution when $u_\text{min}$ and $u_\text{max}$ have different values. We will use a Normal approximation to circumvent this problem. 

##### Normal Approximation 

The parameters $\mu$ and $\sigma$ of the Normal distribution can be written in terms of the mean and standard deviation of the Uniform distribution:

$\begin{align}
\mu &= \frac{u_\text{max}-u_\text{min}}{2}\\
\sigma &= \sqrt{\frac{1}{12}\left(u_{\max}-u_{\min}\right)^2}
\end{align}$

In ACT-R, the minimum and maximum parameters $u_\text{min}$ and $u_\text{max}$ are scaled in terms of mean $\mu$ such that $u_\text{min} = \mu - \frac{\mu}{3}$ and $u_\text{max} = \mu + \frac{\mu}{3}$, respectively. Thus, the standard deviation can be rewritten as:

$\begin{align}
\sigma = \sqrt{\frac{1}{12}\left(\mu + \frac{\mu}{3} - \left(\mu - \frac{\mu}{3}\right) \right)^2} = \frac{\mu}{3}\sqrt{\frac{1}{3}}
\end{align}$

The mean processing time processing times for conflict resolution is $\mu_{\text{cr}}= \gamma$, where $\gamma$ is an adjustable paramter. The mean processing times fo  attention latency, and motor execution are set to their default values: $\mu_{a} = .085$, and $\mu_{\text{me}} = .06,$ respectively.

##### Total Processing Time

Next, we will write two sets of expressions that represent the mean and standard deviation of reaction time for a specified number of production cycles, $n_c$. One set will be for non-attentive response and the other set will be for attentive responses.

The $\mu$ and $\sigma$ parameters for the non-attentive process are defined as:

$\begin{align}
\mu_{\textrm{non-att}}(n_c) = n_c \mu_{\text{cr}} +  \mu_{\text{me}}\\
\sigma_{\textrm{non-att}}(n_c) = \sqrt{n_c \sigma_{\text{cr}}^2 +  \sigma_{\text{me}}^2}
\end{align}$

In each of these equations we multiple the term for conflict resolution by the number of production cycles and add the processing time for motor execution. 

Similarly, the $\mu$ and $\sigma$ parameters for the attentive process are defined as:

$\begin{align}
\mu_{\textrm{att}}(n_c) = n_c \mu_{\text{cr}} + \mu_{a} + \mu_{\text{me}}\\
\sigma_{\textrm{att}}(n_c) = \sqrt{n_c \sigma_{\text{cr}}^2 + \sigma_{a}^2 + \sigma_{\text{me}}^2}
\end{align}$


#### Mixture Weights

As with the processing time distributions, we will define a weighting function for the non-attentive process and the attentive process. For notational simplicity, we will define $A(n)_{s}$, $L(n)_{s}$ and $R(n)_{s}$ as the probability of attending, the probability of a microlapse and the probability of responding given a number of micrlopases $n$. Subscript $s = 1$ for non-attentive state and $s=2$ for an attentive state.

The mixture weight for a non-attentive response is the simplest. Given $n_c$ production cycles, there are $n_c-1$ lapses and one response. Because each function is based on the number of microlapses $n$ rather than the production cycles, we will index the lapse product from $c = \{1,\dots n_c-2\}$. A response occurs after $n_c -1$ lapses. 

$\begin{align}
w_{\textrm{non-att}}(n_c) = R_{1}(n_c-1)\prod_{c=0}^{n_c-2}L_{1}(c)
\end{align}$

The mixture weight for an attentive process is more complex because the attend production rule fires at some point between the first event and the response, which is always the last event. All the ways that the attend production could fire within the sequence is captured by the summation operator. The three terms within the summation represent the joint probability of lapsing before attending $L_{1}(n)$, attending $A_{1}(n)$, and lapsing after attending $L_{2}(n)$. The entire expression is multiplied by $R_{2}(N_c-2)$ because the process always terminates in a response after $N_c -2$ microlapses. 

$\begin{align}
w_{\textrm{att}}(n_c) = R_{2}(n_c-2)\sum_{c=0}^{n_c-2}\left[\left(\prod_{j=0}^{c-1}L_{1}(j)\right) A_{1}(c)\left(\prod_{k=c}^{n_c-3}L_{2}(k)\right)\right]
\end{align}$

#### Probability Density Function 

The number of production cycles is a random variable with no hard upper bound. In principle, there could be an infinite number of production cycles. However, in practice, the PDF can be approximated with with a finite maximum, denoted $N_c$. We found that setting $N_c = 15$, which has an expected processing time of .895 seconds if $\gamma = .05$, works well in many cases. This value is usually sufficient because most reaction times does not exceed .500 seconds. $N_c$ can be increased in cases in which $\gamma$ is smaller and/or reaction times are longer. 

The PDF sums over the number possible number of production cycles $n_c = \left[1,\dots, N_c\right]$ and is broken into two functions: a function for the non-attentive process and a function for the attentive process:

$\begin{align}
f(\textrm{rt}) = \sum_{n_c=1}^{N_c} f_{\textrm{non-att}}(n_c,\textrm{rt}) + f_{\textrm{att}}(n_c,\textrm{rt})
\end{align}$


The function for the non-attentive process weights the processing time for a given number of production cycles:

$\begin{align}
f_{\textrm{non-att}}(n_c,\textrm{rt})) = w_{\textrm{non-att}}(n_c)f_{\textrm{Normal}}(\textrm{rt}) \mid \mu_{\textrm{non-att}}(n_c), \sigma_{\textrm{non-att}}(n_c))
\end{align}$

Similarly, the function for the attentive process weights the processing time for a given number of production cycles:

$\begin{align}
f_{\textrm{att}}(n_c,\textrm{rt})) = w_{\textrm{att}}(n_c)f_{\textrm{Normal}}(\textrm{rt}) \mid \mu_{\textrm{att}}(n_c), \sigma_{\textrm{att}}(n_c))
\end{align}$

Given a set of independent observations, $Y = \{y_1,y_2,\dots,y_{N}\}$, the likelihood function is defined as

$\begin{align}
L(\Theta; Y) = \prod_{i=1}^N f(y_i) 
\end{align}$

### Illustration of Mixture Distribution

In the plot below, each solid distribution corresponds to a component of the mixture for a given number of production cycles. As the number of production cycles increases, the density decreases, but the mean and variance increase. The composite distribution, denoted by the dotted black line, is found by summing the densities of each component distribution at a given value on the x-axis. "
...
# ╔═╡ 2410b274-50a4-4dbe-a20e-2d01340e7d6c
show_cycles(6)
...
The model's data-generating process is defined below in the function called `simulate`, which takes the following arguments

-  $\upsilon$: base utility parameter 
-  $\tau$: utility threshold parameter
-  $\lambda$: utility decrement rate for microlapses
-  $\gamma$: conflict resolution time
- n_trials: number of simulated trials

`simulate` calls the function `simulate_trial` which is responsible for performing the simulation of a single trial. `simulate_trial` is divided into two while loops. The first while loop simulates the production rule selection process until the attend production rule or respond production rule is selected. The state index is updated and the model enters the second while loop where the production rule selection process is simulated until the respond production rule is selected. In each of these steps, the number of microlapses are counted in the variable `n_ml`. See the annotated code for details.  
"

# ╔═╡ f1778bae-a29a-4fbf-948b-a1ede1aa7fe1
md"
The code block below generates 100 simulated trials of the PVT. We will estimate the base utility parameter $\upsilon$ and the fatigue decrement rate parameter, $\lambda$. The fixed parameters are set at typical values for utility threshold $\tau$ and conflict resolution time $\gamma$"

# ╔═╡ 9f5aff05-65ea-4cc1-a34a-c813d6d0bcee
md"
# Log Likelihood Function

The log likelihood is computed using several functions. The functions are presented in three groups, starting with top-level functions and proceeding to low-level functions. 

The first function called is`pvt_log_like`, which computes the mixture weights and parameters for the mixture distributions. These are computed first because they can be reused in the subroutines. Next, the function loops through each reaction time and sums the log likelihoods.

`likelihood_trial` computes the likelihood of a reaction by marginalizing over the non-attentive and attentive processes and all the possible number of production cycles up to `max_cycles`. The density for each mixture component is computed inside the inner for loop. 
"

# ╔═╡ 58462243-45f8-49ba-bd72-d3406759ebaf
md"
The function `mixture_parms` computes the parameters of the normal distribution for all combinations of production cycles and attentive/non-attentive processes. The function outputs `mu` and `sigma`, which are both arrays of size max_cycles X 2.
"
...
To verify the likelihood function has been specified correctly, we will plot the likelihood function against a normalized histogram 10,000 simulated data points. The close correspondence between the density values and the histogram suggest the likelihood function is correctly specified. 
"
...
md"
## Define Model
The prior distributions and model can be summarized as follows:

$\begin{align}
\upsilon \sim \textrm{normal}(4,1) 
\end{align}$

$\begin{align}
\lambda \sim \textrm{beta}(98,2)
\end{align}$
...
# ╔═╡ 187a6b5a-0e43-4551-862e-eb45bc9917ed
...
The diagnostics are generally good. Inspection of the plots below reveals low autocorrelation and no aborations in the trace plots. In the summary above, $\hat{r} \approx 1$ for each parameter, indicating that the chains converged. Note that there are many warnings about numerical errors. Numerical errors can occur during initial sampling for highly constrained parameter distributions, such as $\lambda$. Most of the density for $\lambda$ is between .95 and 1. 

### Posterior Distributions 

The posterior distributions in the third panel of each plot encompass the data generating values ($\upsilon = 4$, $\lambda = .98$), indicating acceptable parameter recovery. 
"

# ╔═╡ 9623cdf6-7833-4dd3-8d01-e766259085d9
md"
#### Posterior Distribution of $\upsilon$
"
...
The plots below compare the posterior predictive distribution of the mean, standard deviation, skewness and lapses, defined as a reaction time greater than .5 seconds, to corresponding values in the data (red line). In each case, the data are near the center of the distributions. This is unsurprising given that the data were generated from the same model. Nonetheless, this provides some evidence that we specified the model correctly. 
"
...
manifest_format = "2.0"
project_hash = "f745783a60f0a9895328fbda8fc4af4aad9a52b8"
...
git-tree-sha1 = "04514e6db6d2169dd63862be54599f52d1c4040e"
...
git-tree-sha1 = "51d0262e5d6b6bd5466c6ea27d78bd8edbb3e84e"
