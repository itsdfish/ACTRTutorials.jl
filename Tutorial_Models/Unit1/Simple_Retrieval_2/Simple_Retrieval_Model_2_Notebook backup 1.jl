...
# Simple Retrieval Model 2

As before, the present model encodes the letter on the screen and submits a retrieval request for the associated number. Unlike Simple Retrieval Model 1, the present model can retrieve the wrong answer in addition to failing to retreive an answer.   
...
\end{align}
```
where $c_{s,\rm imaginal}(\rm letter)$ is the letter value (i.e. A or M) of the chunk in the imaginal buffer. The slot set for the retrieval request is defined as $Q_r = \rm \{letter\}$ 
...
In this section, we will define a rule that maps the result of a retrieval request to an observed response. Let  $y_i$ be the observation on trial $i$ and let $y_i \in \{1,2, \dots, n,\emptyset \}$ be the set possible responses, where $\emptyset$ denotes a retrieval failure. If a chunk is retrieved, then $y_i = c_r(\textrm{number})$ is the observed response. For example, if $c_r(\textrm{number}) = 1$, then the observed answer on trial $i$ is $y_i = 1$.

### Retrieval Probability

Formally, the probability of $y_i$ is:

$\begin{align}
\Pr(Y_i = y_i \mid \delta, \tau, s;\mathbf{r}_i ) = \frac{e^{\frac{\mu_{r}}{\sigma}}}{\sum_{\mathbf{c}_k \in R} e^{\frac{\mu_k}{\sigma}} + e^{\frac{\mu_{m^\prime}}{\sigma}}}
\end{align}$


where $\mu_r$ is the expected activation of the retrieved chunk $r$, $\sigma = s\sqrt{2}$ controls activation noise, and $s$ is the logistic scale parameter (see Weaver 2008).
...
```math
\begin{align}
\mathcal{L}(\delta; \mathbf{Y}, \mathbf{r}_i) = \prod_{i=1}^n \Pr(Y_i = y_i \mid \delta, \tau, s;\mathbf{r}_i )
\end{align}
```
where $\mathbf{Y} = \{y_1,y_2, \dots y_n \}$ is the set of responses. 
"""

# ╔═╡ c122b9a1-fc01-41aa-a1f7-a02240adb08a
md"
## Model Assumptions 
Here we summarize the assumptions of the model:


1. Activation is a function of similarity between the slot values of retrieval request and the slot values of a chunk 
3. A retrieval failure occurs if the activation of all chunks is less than $\tau$
4. No learning or forgeting occurs
"

# ╔═╡ 08d75094-9ed6-4862-b8a4-981989ce61fd
md"
# Generate Data

 In the code block below, we define two functions to generate simulated data from the model: `simulate` and `simulate_trial`. The `simulate` function populates declarative memory with `n_items` chunks, creates a model object, and calls the function `simulate_trial` for each stimulus. Note that the letter slot is omitted from the code for simplicity, but it does not change the predictions. `simulate` accepts the following arguments:
...
`simulate_trial` works by sampling an index from a vector of retrieval probabilities $\theta$ and recording whether the response is correct, incorrect, or a retrieval failure. Without changing the predictions, we ignore the distinction between the cue slot and value slot. Instead we identify whether the retrieval resulted in a correct response, an incorrect response, or a retrieval failure.  
"

# ╔═╡ 823d3bd3-9c16-4b8f-bb9c-f849b42ffd54
md"
Reveal the cell immediately below to see helper functions
"
...
The code block below generates the simulated data
"
...
`computeLL` works in much the same way as the function `simulate`. First, it populates declarative memory with chunks, it creates a model object, and computes the probability of each response category weighted by the number of responses in that category. In order to improve performance, the probabilities for acorrect response, incorrect response, and retrieval failure are cached and reused inside the for loop. 
"

# ╔═╡ cd70b16a-e18d-4298-93ee-4eb60b14d0cd
md"
# Define Model

Our next step is to formulate the model and the prior distribution over $\tau$. In practice, we do not know the exact value of a parameter. However, we typically have some general idea so that we can quantify our uncertainly with a distribution over possible values

```math
\begin{align*}
\tau \sim \rm normal(.5,.5) \\
\delta \sim \rm normal(1, .5)\\
\theta =\Pr(Y_i = y_i \mid \delta, \tau, s;\mathbf{r}_i )\\
y_i \sim \rm Bernoulli(\theta)
\end{align*}
```
...
1. On repetition $i$, sample a parameter vector  $\Theta_j$ from the MCMC chain where $j \in \{1,2,\dots,n_{\textrm{samples}}\}$
2. Simulate data from the model using parameter vector $\Theta_j$
3. Repeat steps 1 and 2 many times

In the code block below, function `posterior_predictive` accepts the data simulation function `simulate`, the chain object, and the number of simulations (e.g. 1000). The histograms below indicate that 40-45 trials out of 50 are correct responses, and most of the incorrect responses are errors of commission. 
"
...
manifest_format = "2.0"
project_hash = "ae1942e9f6511f15968a1568ada00c86d290a983"
