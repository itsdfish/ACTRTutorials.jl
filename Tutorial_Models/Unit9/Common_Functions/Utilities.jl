...
function show_predictions(trial, request, stimulus, slots, parms, sim_fun; add_rts=true, kwargs...)
    ...
    f(;kwargs...) = (request=request,stimulus=stimulus, trial=trial, fans..., mean_rt_yes=mean_rt_yes, mean_rt_no=mean_rt_no, kwargs...)
    vals = map(x->f(chunk=x.slots, act_pm=x.act_pm, act_sa=x.act_sa, act=x.act), results)
