...
In the previous tutorial, we developed the [Simple Retrieval Model](./open?path=$path_u1_1) in which the model responded `Yes` if a memory was retrieved and responded `No` if a retrieval failure occured. In most circumstances, however, guessing is a viable strategy unless there is a explicit penalty for doing so. We will extend the Simple Retrieval Model to allow guessing in the event of a retrieval error. Thus, correct responses are a mixture of retrieval and guessing processes on target trials. Although there are several ways to formulate such a model, we will cast an ACT-R model within the framework of a simple and well-known multinomial processing tree called the 1 High Threshold Model (Erdfelder et al., 2009).  

# Guessing Mixture Model


The 1 High Threshold Model is applied to classic recognition memory tasks in which a participant studies a list of items and later attempts to distinguish the studied items from distractor or foil items. The participant responds `Yes` to indicate if he or she thinks a test item was from the previously studied list and responds `No` otherwise.  The structure of the 1 High Threshold Model is illustrated below, which consists of a sub-tree for target trials and a sub-tree for foil trials. 
")
...
$\begin{align}
\mathbf{r}_i = \{(\textrm{cue},c_{s,\rm imaginal}(\rm cue)\} 
\end{align}$

where $i$ is the trial index, $c_{s,\rm imaginal}(\rm cue)$ is the cue value of the chunk in the imaginal buffer. The slot set for the retrieval request is defined as $Q_r = \rm \{cue\}$ 
...
# ╔═╡ 34f6eb78-3ee4-4f10-8e9d-476b8afeb118
Markdown.parse("
We will adopt the same simplified activation equation as was used in the [Simple Retrieval Model](./open?path=$path_u1_1):
")

# ╔═╡ b03de271-0e7d-475c-a014-e468867769f7
md"
$\begin{align}
...
As before, we will represent the deterministic component of activation as $\mu =$ blc. The approximate probability of retrieving the correct chunk $\mathbf{c}_r$ is given by:

$\begin{align}
     \theta_r = \Pr(\mathbf{c}_r \mid \tau; \mathbf{r}_i)  \approx \frac{e^{\frac{\mu_r}{\sigma}}}{e^{\frac{\tau}{\sigma}} + e^{\frac{\mu_r}{\sigma}}}
\end{align}$

where $\mu_m$ is the deterministic part of activation for chunk $m$.
"

# ╔═╡ 67495140-8340-43ae-943d-22eefe1eb882
md"
## Target Trials

On target trials, the question is a true statement, making the correct answer `Yes`.  The model responds `Yes` when the memory activation is above the threshold. By contrast, if no memory is retrieved, the model engages in a guessing process in which the probability of guessing yes is given by $\theta_{g}$. The complimentary probability of responding `No` is $1-\theta_g$. Thus, on target trials, the model is a <i> mixture </i> of retrieving and guessing. The resulting expression for the probability of responding correctly on a target trial probability is the following mixture:

$\begin{align}
\underbrace{\theta_t}_\textrm{probability yes} = \underbrace{\theta_r}_\textrm{chunk retrieved} + \underbrace{(1-\theta_r) \cdot \theta_g}_\textrm{chunk not retrieved, guess yes}
\end{align}$

The probabilies are summed because the represent mutually exclusive and exhaustive ways of responding `Yes`.
"

# ╔═╡ 5db1949c-bdaa-4713-adff-83aefc1f9dab
md"

## Foil Trials

On foil trials, $\theta_r=0$ because foil items fail to cause activation to exceed threshold. For that reason,  that branch can be be pruned from the model. The model's only recourse is to guess on foil trials. Guessing on foil trials uses the same guessing parameter $\theta_{g}$. 


## Likelihood Function

According to the model, responses are independently and identically distributed for targets and foils. We will define a separate binomial likelihood function for target and foil trials. The likelihood of $k_t$ yes responses out of $n_t$ target trials is:

$\begin{align}
\mathcal{L}_t(\theta_t; k_t, n_t) = {n_t \choose k_t} \theta^k_t (1-\theta_{t})^{n_t-k_t} 
\end{align}$

Similarly, the likelihood of $k_f$ yes responses out of $n_f$ foil trials is:

$\begin{align}
\mathcal{L}_f(\theta_g; k_f, n_f) = {n_f \choose k_f} \theta_{g}^{k_f} (1-\theta_{g})^{n_f-k_f} 
\end{align}$

The likelihood function across all trial types is given as the product of likelihoods:

$\begin{align}
\mathcal{L}_t(\theta_t, \theta_g; k_t, n_t, k_f, n_f)  = \mathcal{L}_t(\theta; k_t, n_t)  \mathcal{L}_f(\theta_g; k_f, n_f)  
\end{align}$
...
1. The correct memory is retrieved if its activation is above $\tau$
2. If a memory cannot be retrieved, the model guesses `Yes` with probability $\theta_g$
3. All trials are independent
4. No learning occurs 
...
# ╔═╡ 349c7631-2851-4906-a5f2-b05d8b06c577
md"
# Generate Data

We will use the following function to generate data for parameter estimation. The `simulate` function accepts the following arguments:

- parms: a `NamedTuple` of fixed parameters
- N: he number of simulated trials
-  $\tau$: the retrieval threshold parameter
-  $\theta_g$: the parameter for guessing `Yes`

The annotated code below shows how an an ACTR object is created and the number of correct responses are generated for both target and foil trials. As you can see, the data generation process is similar to that of the Simple Retrieval Model, except (1) there are target and foil trials and (2) the probability of responding `Yes` on a target trial is a mixture of retrieval and guessing processes. 
"

# ╔═╡ 3303fd9a-78e6-4185-8df5-48cd5b01591a
...
md"
Now that our `simulate` function has been defined, we can now generate some data. We will set the number of target trials to 80 and the foil trials to 20. The  retrieval threshold parameter $\tau$ and the guessing bias parameter $\theta_g$ will be estimated while the parameters in fixed_parms will be fixed. 
"
...
In the code below, we define a custom distribution object for the Guessing Mixture model, which contains fields

-  $\tau$: retrieval threshold parameter
-  $\theta_g$: probability of a correct guess
- parms: a `NamedTuple` of fixed parameters. 

The function `computeLL` performs the heavy lifting and is called via `logpdf` when the Retrieval distribution object is passed. As you can see, `computeLL` has a similar set up to the data simulation function. The primary difference is that it evaluates the log likelihood of the data rather than generating the data. 
"
...
$\theta_{g} \sim \rm beta(8,2)$

$\theta_{r} = \rm Pr(R=\textrm{yes} ;\textrm{target})$

$\theta = \theta_{r} + (1-\theta_r)\theta_g$

$R_{t} \sim \rm binomial(N_{t},\theta)$

$R_{f} \sim  \rm binomial(N_{f},\theta_g)$ 

where $R_{t}$ and $R_{f}$ are the number of `Yes` responses on target and foil trials, respectively, $N_{t}$ and $N_{f}$ are the number of target and foil trials, respectively, $\theta_g$ is the guessing parameter, which is set to the base rate of target trials ($80\%$) to reflect probability matching, and $\theta$ is the probability of responding `Yes` in target trials. Importantly, $\theta$ consists of a retrieval component and a guessing component. 

The Turing model looks similar to the mathematical discription above. 
"

# ╔═╡ ab1416e8-202a-4f43-a6be-da79fb645c15
...
Recall that the data generating parameters were $\theta_g = .8$ and $\tau = .5$. The summary above shows that the mean posterior distribution is centered near the data generating parameters. This can be confirmed in the density plots below. It is also worth nothing that there is only a small reduction in uncertainty in the posterior distribution of $\tau$ relative to its prior distribution. On the other hand, the standard deviation of the posterior of $\theta_g$ is approximately half the corresponding standard deviation of the prior distribution. This is due, in part, to the fact that $\theta_g$ appears in the equation for both target and foil trials, thereby providing more information for parameter estimation. 

### Diagnostics

The first panel for each plot shows good mixing between the four chains. In the second panel for each plot, the auto-correlation between successive samples drops rapidly after a lag of about 5, indicating a high effective sample size.
"
...
The posterior predictive distributions of correct responses are shown below for target and foil trials. Both distributions show high accuracy with negatively skewed posterior predictive distributions. The model predicts slightly lower accuracy for foil trials compared to target trials. 
"
...
manifest_format = "2.0"
project_hash = "6de2e67a282fefd4a2d232b8677c350f3855422f"
