...
The goal of this tutorial will be to develop a simple model of serial recall. Serial recall involves learning a list of items and attempting to recall the items in the same order in which they were learned. Transposition errors, recency effects, and primacy effects are three common phenomena found in serial recall experiments. In this tutorial, we will focus on generating transposition errors. In the [next tutorial](Serial_Recall_2/Serial_Recall_Model_2.ipynb), we will include a mechanism to produce primacy effects. Producing recency effects will be left as an optional exercise for you to complete.
"""

# ╔═╡ 7bf577bd-767e-4a57-b9ef-f390d8404872
md"""
## Transposition Errors

A transposition error occurs when the order of items are incorrectly recalled. Transposition errors are more likely to occur between words that are close to each other in terms of order as opposed to further apart. The table below provides two illustrations of a transposition error. In Example 1, the second and third words are listed in reverse order. This type of transposition error is more likely to occur than the transposition error in Example 2, which involves swapping the order of the second and seventh words.
"""

# ╔═╡ 560e9ef8-efb8-4613-a7fe-36011e1cee0b
md"""

|     Order      | 1   | 2     | 3     | 4    | 5    | 6    | 7    |
|-----------|-----|-------|-------|------|------|------|------|
| Correct   | car | dog   | table | file | bolt | leaf | sock |
| Example 1 | car | **table** | **dog**   | file | bolt | leaf | sock |
| Example 2 | car | **sock**  | table | file | bolt | leaf | **dog**|
"""

# ╔═╡ be8dade6-228e-4fa9-9106-153a591a4fc7
md"""

## Serial Position Effects

Serial recall is characterized by two types of serial order effects: the primacy effect and the recency effect. When the primacy and recency effect co-occur, the relationship between accuracy and position is U-shaped.

### Primacy Effect

The primacy effect occurs when items at the begining of a list are more likely to be correctly ordered than items in the middle of the list.


### Recency Effect

The recency effect occurs when items at the end of a list are more likely to be correctly ordered than items in the middle of the list. 


# Serial Recall Task

In the serial recall task, participants learn an ordered list of words and attempt to recall the list in the presented order. During each training block, each word from the list is presented for 2 seconds in the correct order. There are a total of 10 training blocks in the training phase.


# Serial Recall Model 1

Our goal is to build up to a model that accounts for transposition errors, the recency effect and the primacy effect. We will begin with a simple model that only accounts for transposition errors. In a separate tutorial, we will develop a more complete model that accounts for all three phenomena. 

The model begins by encoding the study list into memory. During the test phase, the model performs successive memory retrievals using position $i \in \{1,2,\dots, N\}$ as a cue or retrieval request for recalling the list in the correct order. Activation is a function of similarity to position $i$ in the memory retrieval request. Thus, chunks with a similar position slot value are more likely to be confused than chunks with a very different position value. 


## Declarative memory

Declarative memory $M$ consists of a chunk for each studied word. Each chunk contains a slot for the word, a slot for its position within the study list, and a slot that indicates whether the chunk has been retrieved during the test phase. As an example, consider the chunk $m$:
```math
\begin{align}
\mathbf{c}_m = \{\rm (word,bicycle),(position,2), (retrieved,0)\}
\end{align}
```
where the slots (i.e., domain) are $Q = \{\textrm{word, position, retrieved}\}$. The slot "retrieved" uses numerical indices 0 to represent false and 1 to represent true.
"""

# ╔═╡ 0b630329-40d2-49af-9a93-f07b2a2d1ae0
md"""
## Inhibition of Return

ACT-R uses the slot "retrieved" as a deterministic mechanism for inhibition of return to prevent previously retrieved words from being retrieved more than once during the test phase. Thus, when a retrieval request is submitted to declarative memory, previously retrieved chunks are excluded from the list of candidate chunks. To this end, we define $R$ as the set of chunks that have not been retrieved:
...
The retrieval request for position $i$ is defined as

$\begin{align}
\mathbf{r}_i = \rm \{(position,i),(retrieved,0)\} 
\end{align}$

The slot set for the retrieval request is defined as $Q_r = \rm \{position,retrieved\}$ 
...
Activation for the Serial Recall Model 1 is defined as:

$\begin{align}
    a_{m} = \text{blc} + \rho_m + \epsilon_m
\end{align}$

where the partial matching term is given as

$\begin{equation}
\rho_m  = -\delta \sum_{q \in Q_r}| c_m(q) - r_i(q) | 
\end{equation}$

Partial matching produces graded transposition errors because it is based on the absolute difference between a chunk's position and the position in the retrieval request. As the displacement increases, so too does the penalty. Mismatch penalty is plotted as a function of displacement relative to a position of 5 for three different values of $\delta$. The minimum of the gradient for each $\delta$ is found at position 5 and increases linearly in each direction.
"""
...
In the annotated code below, the model objects and data array are initialized at the beginning of the function `simulate`. In the for loop, the index `i` serves as the position slot value for the retrieval request in `retrieval_probs`. Once the retrieval probabilities are computed, a chunk index `resp` is selected from the multinomial distribution. Finally, the retrieved chunk is modified so that its retrieved slot value is set to true and the position of the retrieved chunk is recorded in the `NamedTuple` for this trial. For simplicity, the word slot is omitted from the chunks because the position index provides the same information more convienently.  
"""
...
The density plot below show shows the posterior distribution is centered near the data generating of $\delta = 1$. As the summary above indicates, the standard deviation of the posterior distribution is nearly 5 times smaller than the standard deviation of the prior distribution, indicating a large reduction in uncertainty.
...
The code block below generates the posterior distribution of transposition displacement (i.e. absolute difference between the retrieved and correct position). As expected, transposition errors decrease with position displacement. A a displacement of zero, the model retrieves the correct item with a probability of approximately 71%. Importantly, the probability decreases sharply with increasing displacement.
"""
...
md"""
It turns out that inter-trial dependencies from inhabition of return can produce a small primacy effect. In the code block below, inhabition of return is removed from the model by removing`retrieved=[false]` from the retrieval request passed to `retrieval_probs`. Running this block will overwrite `simulate` with the new definition. In the plot below, the line no longer has a downward slope. You may have noticed something peculiar: accuracy dips slightly in the middle compared to the end points. To be clear, this is not the full serial position effect. It is a consequence of asymetry in the error distribution at the end points. In particular, the item in the first position can only be confused with items in a later position, resulting in a lower error rate. This can be seen in the right hand plot, which shows the probability of retrieving different items given a retrieval request for the item in the first position. By changing $x=5$A similar line of reasoning applies to the item in the last position.
"""
...
manifest_format = "2.0"
project_hash = "9c437d6975c5c21bfc5a409cbcf9ee2807a93392"
