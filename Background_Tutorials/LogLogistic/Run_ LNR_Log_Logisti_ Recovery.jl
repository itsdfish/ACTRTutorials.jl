...
deterministic = true
μt = [rand(Uniform(-1.5, 1.5), Nr) for i in 1:n_sim]
st = rand(Uniform(.3, 1), n_sim)
parms = [Symbol("tμ", i) for i in 1:Nr]
...
μta = hcat(μt...)'
coefs = DataFrame(parm=Symbol[], intercept=Float64[], slope=Float64[], correlation=Float64[])
parms = [Symbol("μ",i) for i in 1:Nr]
push!(parms, :s)
for (i,parm) in enumerate(parms)
    ...
    @df results scatter!(cols(θt), cols(θe), xlabel=string(parm," true"), ylabel=string(parm," estimate"), grid=false,
        color=:grey, leg=false, subplot=i, markersize=3.5,markerstrokewidth=1, xlims=(lb,ub), ylims=(lb,ub),
